...
The latter was goal of the present study and completed by $N=`r sum(data_survey[,"followup_ered_complete"] == 2)-1`$ participants. 
...
It was nevertheless surprising that the strategy suppression showed on average the lowest *corrugator* activity, the lowest effort ratings and the highest utility ratings. 
...
Contrary to our hypothesis H6, utility was the best predictor for individual SVs, explaining `r papaja::printnum(H5_M2_table[4,5]*100, digits = 1)` % of variance in SVs.
...
This is consistant with the literature, which has already reported correlations between self-control and demand avoidance[@Kool2013].
...
<!-- Argumentation Corrugator nicht zw. Lvl unterschiedlich, aber trotzdem einfluss, weil man da eine relativ direkte Rückmeldung hat + Levator eher nicht -> alle Bilder negativ aber nur einige eklig, wenn man also nur diese Bilder reinnimmt, wird vielleicht Levator signifikant; 
Effort Ergebnisse bestätigen ESER Paper, dass man nach Effort-Minimierung strebt;
Utility: Subjektive Einschätzung der Strategie als Mittel zur Zielerreichung, was eine überschneidung mit dem SV darstellt und außerdem evtl. eine Interaktion von Effektivität und Effort -->
