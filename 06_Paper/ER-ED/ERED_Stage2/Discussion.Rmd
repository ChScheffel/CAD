...
The measures of arousal and effort also showed high variability between individuals.
...
In the case of the EMG measurement, this could be due to the fact that the result of the implementation of the instruction ("Maintain a neutral facial expression") is measured directly. 
The direct instruction also reduces the complexity of the generation process.
This could be why the required subjective effort is the lowest.
Finally, immediacy and simplicity are expressed as greater subjective utility of the strategy suppression.
In addition, one receives relatively direct feedback from one's own facial muscle activity as to how well suppression has been implemented.
In contrast, the strategies distraction and distancing require a more detailed evaluation of internal states in order to assess their utility, which in turn requires more effort.
...
However, this would require at least two measurements of SVs.
...
However, we chose this amount because, firstly, we followed the original paradigm of Westbrook[@Westbrook2013], and secondly, it has been shown in the context of cognitive effort discounting, that a lower incentive increases participants' sensitivity to effort differences[@Bialaszek2017].
