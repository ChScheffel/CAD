...
Data collection took place between `r format(as.Date(acqui_time[1]), "%Y-%m")` and `r format(as.Date(acqui_time[2]), "%Y-%m")`. <!-- hier wird "2022-08 and 2023-02" geschrieben, finde ich im Lesefluss nicht so schön. Wie wäre es mit "August 2022 and February 2023"?-->
...
### Manipulation checks <!-- es gibt keine weitere Überschrift auf dieser Ebene, würde ich jedoch erwarten.-->

#### Subjective arousal and physiological responding <!-- ich würde empfehlen, die selben Überschriften wie in 2.5 Analysis Plan zu nehmen, das wäre für diese Ergebnisse "Effect of valence and emotion regulation on arousal and facial EMG." -->

To explore whether negative pictures evoke emotional arousal and physiological responding, we conducted separate rmANOVAs for the active viewing condition with predictors subjective arousal, *Corrugator* and *Levator* activity. <!-- in den Methods habt ihr es klein und kursiv geschrieben. -->
...
To investigate whether ER strategies reduce emotional arousal and physiological responding, we conducted separate rmANOVA comparing the four instructed strategies (active viewing, distraction, distancing, suppression) with respect to subjective arousal, *corrugator* and *levator* activity. 
...
#### Subjective effort of ER strategies <!-- ich würde empfehlen, die selben Überschriften wie in 2.5 Analysis Plan zu nehmen, das wäre für diese Ergebnisse "Effects of emotion regulation on effort" -->
...
Cognitive effort also played the most important role in the subsequent choice decision, which resembled previous findings of our group[@Scheffel2021]. 
`r round((54/119)*100, digits = 1)`% of the participants stated that they chose the strategy that was easiest for them to implement.
...
A more detailed list of all reasons can be found in Table XX <!-- Nummer noch ergänzen --> in [supplementary material](#SupplementReasonChoice).

### Neue Überschrift? <!-- ab hier sind es doch keine Manipulation Checks mehr, oder? -->
...
$n=$ `r  papaja::printnum(sum(data_SV$sv == 1))` had one SV of 1.0, indicating a clear preference for one ER strategy.
...
The preregistered model showed a correlation of $r=$ `r papaja::printnum(attr(summary(MLM_1)$varcor[1]$ID,"correlation")[1,2])` between the random effects subjects and recoded strategy ($BF10$ of the variable strategy: `r papaja::apa_print(MLM_1_BF)$statistic`). <!-- für die Lesbarkeit wäre es besser, wenn die Variablen irgendwie anders formatiert wären, kursiv oder in Anführungszeichen. -->
Our model explained `r papaja::printnum(MLM1_r2[2])`% of variance <!-- im PDF steht dasnn 0.90% of variance, aber ihr meint doch 90% of variance, oder? --> and thus we assumed our model was overfitted due to including recoded strategy as the random slope.
...
The second model explained `r papaja::printnum(MLM2_r2[2])`% of variance <!-- im PDF steht dasnn 0.41% of variance, aber ihr meint doch 41% of variance, oder? --> . 
...
The values <!-- welche? Beta? --> were relatively small, so the respective effect size $f^{2}$ was calculated as the explained variance.
...
The estimated odds ratio indicated a higher chance of choosing distraction, when the SV of that strategy is higher.
Additionally, the SV of the strategy suppression contributed significantly to the model (`r papaja::apa_print(Choice_OrdReg)$full_result$suppression_sv`, $BF10=$ `r papaja::printnum(Choice_OrdReg_Posterior$BFtu_exploratory[4,3])`).
...
The starting point for this was the model reported before. <!-- welches model genau? -->
Only predictors that had previously shown a significant association with SVs were included in the model together with level-2 predictors self-control and NFC. <!-- ist das wirklich zulässig? Ich hab mich gefragt, ob das nich zu Kritik von den Reviewern führen könnte, dass ihr hier nur die vorher sign. predictors auswählt.-->
