...
The procedure described above was tested in a pilot study with $N=`r nrow(data_quest_pilot)`$ participants (`r length(which(data_quest_pilot$gender == 2))` female; age: $M = `r papaja::printnum(mean(data_quest_pilot[,"age"], na.rm = TRUE), digits = 1, gt1 = TRUE)`\text{ }\pm\text{ }SD = `r papaja::printnum(sd(data_quest_pilot[,"age"], na.rm = TRUE), digits = 1, gt1 = TRUE)`$).
<!-- direkt "above" wird das nicht beschrieben, die genaue procedure wird eigentlich auch erst unten beschrieben. Ich würde eher schreiben "The newly developed ER paradigm was tested in a pilot study" oder so ähnlich. --> 
...
<!-- Signifikanz beruht auf p-Werten? Würde ich angeben. --> 
...
<!-- kann man die Results irgendwo nachvollziehen? Ich würde zumindest MW, SD und p-Werte angeben wenn gesagt wird, dass es "signifikant" höher/nierdiger ist. Auch um zu zeigen, worauf sich die Aussage stützt. 
könnte man die Ergebnisse sogar in einer Tabelle/Abbildung darstellen? Je nachdem, wie wichtig die Pilotdaten für das Gesamt-Ms. sind... 

Ich war etwas verwirrt dass die Pilotdaten schon hier kommen. Ich hätte erwartet, dass erst das Paradigma genau beschrieben wird, und dann als "Schmankerl" die ersten Pilotdaten mitgeliefert werden. Evtl. mit einer extra Überschrift "Introduction / Methods / Pilot study (oder data)". So gehen sie m.E. etwas unter, weil man sich nachfolgend wieder auf die Details des Paradigmas konzentriert. Hier wurde aber auch nicht alles untersucht, wie z.B. Fragebögen? --> 
...
Participants will receive 30.00€ or course credit as compensation.
<!-- das steht schon bei Ethics information, evtl. kürzen. -->
...
<!-- Hier und im Folgenden wird wieder Präsens verwendet - vereinheitlichen? -->
To measure resilience, the German version 10-item-form of the Connor-Davidson resilience Scale (CD-RISC)[@Connor2003,@Campbell-Sills2007; @Sarubin2015] is used.
Habitual use of ER is assessed using the German version of the Emotion Regulation Questionnaire (ERQ) [@GrossJohn2003; @Abler2009].
For the assessment of ER flexibility we use the Flexible Emotion Regulation Scale (FlexER) [@Doerfel2019].
...
For more detailed information on psychometric properties of the questionnaires, please see [supplementary material](#SupplementQuestionnaires). <!-- Not sure whether you can state this so briefly without giving more detailed information on, e.g., sample items, the scale on which answers are recorded (e.g., -3 ... +3), the reliability of the measures. 
AG: im Paper würde man das hier angeben, bei RR reicht vielleicht Supplememt? -->

### 2.3.2 Emotion regulation paradigm

The ER paradigm consists of three parts that will be described in the following.
...
During suppression, participants are told to "suppress their emotional facial expression." They should imagine being observed by a third person that should not be able to tell by looking at the facial expression whether the person is looking at an emotional picture.
...
The procedure of the discounting follows the COG-ED paradigm by Westbrook et al. [-@Westbrook2013] with a major change.
...
Participants are instructed to decide as realistically as possible by imagining that the money displayed is actually available for choice.
...
Importantly, there is no further instruction on what basis they should make their decision.
...
Bipolar facial electromyography (EMG) will be measured for *corrugator supercilii* and *levator labii* as indices of affective valence [@Bradley2000], similar to previous work by our group [@Gärtner, Jawinski, Strobel, 2021].
<!-- da wir das ja sogar schon gemacht haben, fände ich es cool die Studie zu erwähnen. :-) Zeigt ja auch, dass wir keine Neulinge auf dem Gebiet sind. Referenz muss glaub noch in Bib eingefügt werden! --> 
...
Detailed information on power calculation for each hypothesis can be found in the [design table](#DesignTable) in the Supplement.
<!-- hier würde ich den Verweis zur Design Table ganz explizit machen, also sagen, wo diese zu finden ist. -->

## 2.5 Analysis plan
<!-- hier wird wieder durchgehend Futur verwendet - würde das in Methods vereinheitlichen. -->
...
<!-- Bei den krusiven Zwischenüberschriften wie in der nächsten Zeile würde ich Punkte setzen, sieht sonst im PDF sehr eng zusammen aus. ;-)  -->
...
For each result of the analyses, both $p$-values and Bayes factors $BF10$, calculated using the *BayesFactor* package [@Morey2021], will be reported. Bayes factors are calculated using the default prior widths of the functions *anovaBF*, *lmBF* and *regressionBF*.
