...
SubjArousalView_BF <- BayesFactor::anovaBF(formula = arousal ~ block,
                                           data = Ratings_view,
                                           progress = FALSE)
SubjArousalView_con$BF10 <- BayesFactor::extractBF(BayesFactor::ttestBF(x = Ratings_view$arousal[Ratings_view$block == "view_neu"],
                                                                        y = Ratings_view$arousal[Ratings_view$block == "view_neg"],
...
SubjArousalView_con[1,1] <- "$View_{negative} - View_{neutral}$"
...
SubjArousalReg_con$BF10 <- c(BayesFactor::extractBF(BayesFactor::ttestBF(x = Ratings_reg$arousal[Ratings_reg$block == "view_neg"],
                                                                         y = Ratings_reg$arousal[Ratings_reg$block == "distraction"],
                                                                         progress = FALSE, paired = TRUE))$bf,
                             BayesFactor::extractBF(BayesFactor::ttestBF(x = Ratings_reg$arousal[Ratings_reg$block == "view_neg"],
                                                                         y = Ratings_reg$arousal[Ratings_reg$block == "distancing"],
                                                                         progress = FALSE, paired = TRUE))$bf,
                             BayesFactor::extractBF(BayesFactor::ttestBF(x = Ratings_reg$arousal[Ratings_reg$block == "view_neg"],
                                                                         y = Ratings_reg$arousal[Ratings_reg$block == "suppression"],
                                                                         progress = FALSE, paired = TRUE))$bf,
                             BayesFactor::extractBF(BayesFactor::ttestBF(x = Ratings_reg$arousal[Ratings_reg$block == "distraction"],
                                                                         y = Ratings_reg$arousal[Ratings_reg$block == "distancing"],
                                                                         progress = FALSE, paired = TRUE))$bf,
                             BayesFactor::extractBF(BayesFactor::ttestBF(x = Ratings_reg$arousal[Ratings_reg$block == "distraction"],
                                                                         y = Ratings_reg$arousal[Ratings_reg$block == "suppression"],
                                                                         progress = FALSE, paired = TRUE))$bf,
                             BayesFactor::extractBF(BayesFactor::ttestBF(x = Ratings_reg$arousal[Ratings_reg$block == "distancing"],
                                                                         y = Ratings_reg$arousal[Ratings_reg$block == "suppression"],
                                                                         progress = FALSE, paired = TRUE))$bf)
...
SubjArousalReg_con[1,1] <- "$Distancing - Distraction$"
SubjArousalReg_con[2,1] <- "$Distancing - Suppression$"
SubjArousalReg_con[3,1] <- "$Distancing - View_{negative}$"
SubjArousalReg_con[4,1] <- "$Distraction - Suppression$"
SubjArousalReg_con[5,1] <- "$Distraction - View_{negative}$"
SubjArousalReg_con[6,1] <- "$Suppression - View_{negative}$"
...
SubjEffort_con$BF10 <- c(BayesFactor::extractBF(BayesFactor::ttestBF(x = Ratings_reg$effort[Ratings_reg$block == "view_neg"],
                                                                         y = Ratings_reg$effort[Ratings_reg$block == "distraction"],
                                                                         progress = FALSE, paired = TRUE))$bf,
                             BayesFactor::extractBF(BayesFactor::ttestBF(x = Ratings_reg$effort[Ratings_reg$block == "view_neg"],
                                                                         y = Ratings_reg$effort[Ratings_reg$block == "distancing"],
                                                                         progress = FALSE, paired = TRUE))$bf,
                             BayesFactor::extractBF(BayesFactor::ttestBF(x = Ratings_reg$effort[Ratings_reg$block == "view_neg"],
                                                                         y = Ratings_reg$effort[Ratings_reg$block == "suppression"],
                                                                         progress = FALSE, paired = TRUE))$bf,
                             BayesFactor::extractBF(BayesFactor::ttestBF(x = Ratings_reg$effort[Ratings_reg$block == "distraction"],
                                                                         y = Ratings_reg$effort[Ratings_reg$block == "distancing"],
                                                                         progress = FALSE, paired = TRUE))$bf,
                             BayesFactor::extractBF(BayesFactor::ttestBF(x = Ratings_reg$effort[Ratings_reg$block == "distraction"],
                                                                         y = Ratings_reg$effort[Ratings_reg$block == "suppression"],
                                                                         progress = FALSE, paired = TRUE))$bf,
                             BayesFactor::extractBF(BayesFactor::ttestBF(x = Ratings_reg$effort[Ratings_reg$block == "distancing"],
                                                                         y = Ratings_reg$effort[Ratings_reg$block == "suppression"],
                                                                         progress = FALSE, paired = TRUE))$bf)
...
SubjEffort_con[1,1] <- "$Distancing - Distraction$"
SubjEffort_con[2,1] <- "$Distancing - Suppression$"
SubjEffort_con[3,1] <- "$Distancing - View_{negative}$"
SubjEffort_con[4,1] <- "$Distraction - Suppression$"
SubjEffort_con[5,1] <- "$Distraction - View_{negative}$"
SubjEffort_con[6,1] <- "$Suppression - View_{negative}$"
...
We used *R* with *R Studio* [@RCore2021; @RStudioTeam2020] with the main packages ... <!-- name and reference the main analysis packages, agian for consistentcy with Zerna et al. --> for all our analyses.
...
<!-- So liest es sich in Josephines Ms. und so ähnlich liest es sich in publizierten NHB papers. -->
...
The procedure described above was tested in a pilot study with $N=`r nrow(data_quest)`$ participants (`r length(which(data_quest$gender == 2))` female; age: $M = `r round(mean(data_quest[,"age"], na.rm = TRUE), digits = 1)`\text{ }\pm\text{ }SD = `r round(sd(data_quest[,"age"], na.rm = TRUE), digits = 1)`$).<!-- This results in two digit numbers, with the last digit being zero. Perhaps use papaja::printnun(x, diguts = 1, gt1 = T) instead, this rounds up to 1 digit, but also only displays one digit (at least I hope so) -->
...
Next, electrodes to measure EMG <!-- Has abbrev. EMG already are been introduced? -->being attached and the ER paradigm is conducted.
...
<!-- somehow refer to the Dign Table here, where everything is detailed -->

## 2.5 Analysis plan

All statistical analyses will be performed using *RStudio* (version 1.4.1717) [@RStudioTeam2020]and *R* (version 4.1.0) [@RCore2021] for Windows.<!-- mention the main packages used and list the helper packages in the supplement -->
...
To investigate the effect of the tree ER strategies on subjective arousal, another rmANOVA will be conducted with the factor strategy (active viewing, distraction, distancing, and suppression) for subjective arousal ratings of negative pictures.
To examine the impact of valence on the emotional facial reaction, an rmANOVA with the factor valence (neutral and negative) for strategy active viewing will be conducted for EMG activity of *corrugator* and *levator*.
<!-- @Anne please check! =) --> A further rmANOVA with the factor strategy (active viewing, distraction, distancing, and suppression) will be conducted for EMG activity to examine the effects of ER strategies on the emotional facial reaction.
<!-- @Anne please check! =) --> To examine the effect of ER strategies on cognitive effort, an rmANOVA with the factor strategy (active viewing, distraction, distancing, and suppression) for subjective effort ratings will be conducted.
...
<!-- perhaps alsway mention which r package will be used to run these proceudres -->
*Subjective values of emotion regulation strategies* <br> For each ER strategy, SVs will be calculated as follows: first, the value 0.015625 will be added to or subtracted from the last monetary value of the flexible strategy, depending on the participant's last choice.<!-- why exactly this value? -->
...
<!--CS: Weitere Persönlichkeitsfaktoren ins Modell? Würde sagen erstmal nicht, denn sonst wird es a) überladen mit Prädiktoren und b) haben wir sonst kein Maß, was meiner Meinung nach mit ALLEN SVs zusammenhängen sollte. Anm. AS: Ja, erstmal so schlank wie möglic halten. -->
...
<!--CS: Reicht das so als Beschreibung und wird klar, woher die Slopes und Intercepts kommen werden? AS: Mir ehrlich gesagt erstmal nicht so ... -->

For each result of the analyses both, $p$-values and Bayes factor $BF10$, calculated using the *BayesFactor* package [@Morey2021], will be reported. Bayses factors are calculated using the default prior widths of the functions *anovaBF*, *lmBF* and *regressionBF*. <!-- bzw welche Funktionen auch immer Du nutzt, hoffe, das reicht denen, denn wenn man genauer werden will, müsste man noch zwischen fixed und random effects priors unterscheiden, die zu allem Überdruß auch noch unterschiedliche Zahlenwerte haben, 1/2 für fixed und 1 für random bei anovaBF und lmBF, aber sqrt(2)/4 bei regressionBF -->
