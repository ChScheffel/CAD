...
EMGCorrView_BF <- BayesFactor::anovaBF(formula = Corr ~ block,
                                       data = EMG_view,
                                       progress = FALSE)
EMGCorrView_con$BF10 <- BayesFactor::extractBF(BayesFactor::ttestBF(x = EMG_view$Coor[EMG_view$block == "view_neu"],
...
# figure
FigEMGCorrViewPilot=ggplot2::ggplot(EMG_view, aes(x = block, y = arousal))+
  geom_boxplot(width = 0.7, position = position_dodge(0.8))+
  scale_x_discrete(name = "Active viewing", 
                   limits = c("view_neu", "view_neg"),
                   labels = c("Neutral", "Negative"))+
