...
data_ER = data.frame(ID = character(), block = character(), arousal = double(), effort = double(), trigger_view = double(), trigger_reg = double())
...
for (i in 1:nrow(data_ER)) {
  ...
  if (data_ER$trigger_view[i] == 21 & is.na(data_ER$trigger_reg[i])) {
    
    data_ER$block[i] = "view_neu"
    
  } else if (data_ER$trigger_view[i] == 22 & is.na(data_ER$trigger_reg[i])) {
    
    data_ER$block[i] = "view_neg"
    
  } else if (is.na(data_ER$trigger_view[i]) & data_ER$trigger_reg[i] == 23) {
    
    data_ER$block[i] = "distraction"
    
  } else if (is.na(data_ER$trigger_view[i]) & data_ER$trigger_reg[i] == 24) {
    
    data_ER$block[i] = "distancing" 
    
  } else if (is.na(data_ER$trigger_view[i]) & data_ER$trigger_reg[i] == 25) {
    
    data_ER$block[i] = "suppression"
  
  } else if (is.na(data_ER$trigger_view[i]) & is.na(data_ER$trigger_reg[i])) {
...
aov.arousal = afex::aov_ez(id = "ID",
                           dv = "arousal",
                           data = data_ER,
                           within = "block",
                           fun_aggregate = mean) 
afex::nice(aov.arousal, es = "pes")

# compute posthoc tests for within measures
posthoc_arousal = emmeans::emmeans(aov.arousal$aov, specs = "block")
pairs(posthoc_arousal, adjust = "bonferroni")
