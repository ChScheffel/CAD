...
The association between flexible ER and SVs of ER strategies (H7b) will be investigated with a linear regression using the individual $intercept$ (i.e., the extent to which an invididual considers any of the ER strategies useful) and $slope$ (i.e., the extent to which one strategy is preferred over others, indicating less flexibility) of each participants' SVs to predict their FlexER score.
To estimate individual $intercepts$ and $slopes$, SVs will first be sorted by magnitude in descending order.
Values will be fitted in a linear model to estimate individual $intercepts$ and $slopes$.
