...
data_EMG$block[data_EMG$trigger == " S 21"] <- "view_neu"
data_EMG$block[data_EMG$trigger == " S 22"] <- "view_neg"
data_EMG$block[data_EMG$trigger == " S 23"] <- "distraction"
data_EMG$block[data_EMG$trigger == " S 24"] <- "distancing"
data_EMG$block[data_EMG$trigger == " S 25"] <- "suppression"
data_EMG$block[data_EMG$trigger == " S 26"] <- "choice"
...
EMG_view <- data_EMG %>%
  subset(data_EMG$block == "view_neu" | data_EMG$block == "view_neg")
...
EMGCorrView_BF <- BayesFactor::anovaBF(formula = Corr ~ block,
                                       data = EMG_view,
                                       progress = FALSE)
EMGCorrView_con$BF10 <- BayesFactor::extractBF(BayesFactor::ttestBF(x = EMG_view$Corr[EMG_view$block == "view_neu"],
                                                                    y = EMG_view$Corr[EMG_view$block == "view_neg"],
                                                                    progress = FALSE, paired = TRUE))$bf
...
# rename contrast for table
EMGCorrView_con[1,1] <- "$View_{negative} - View_{neutral}$"
...
EMGLevView_BF <- BayesFactor::anovaBF(formula = Lev ~ block,
                                       data = EMG_view,
                                       progress = FALSE)
EMGLevView_con$BF10 <- BayesFactor::extractBF(BayesFactor::ttestBF(x = EMG_view$Lev[EMG_view$block == "view_neu"],
                                                                    y = EMG_view$Lev[EMG_view$block == "view_neg"],
                                                                    progress = FALSE, paired = TRUE))$bf
...
# rename contrast for table
EMGLevView_con[1,1] <- "$View_{negative} - View_{neutral}$"
...
Bipolar facial electromyography (EMG) will be measured for *corrugator supercilii* and *levator labii* as indices of affective valence (Bradley & Lang, 2000).
...
Impedance level will be kept below 10 kΩ. 
...
Corrugator and Levator EMG will be analyzed during the 6 s of picture presenation.
...
<!-- @Anne please check! =) --> A further rmANOVA with the factor strategy (active viewing, distraction, distancing, and suppression) was conducted for EMG activity to examine 
<!-- AG: hier ein Check!-->
the effects of ER strategies on the emotional facial reaction.
