...
We used \emph{R} with \emph{R Studio}\textsuperscript{20,21} with the main packages \ldots{} for all our analyses.
...
Young, healthy participants (aged 18 to 30 years) will be recruited using the software \emph{ORSEE}\textsuperscript{22} at the Technische Universität Dresden.
...
Study data are being collected and managed using REDCap electronic data capture tools hosted at Technische Universität Dresden.\textsuperscript{23,24}

\hypertarget{psychometric-measures}{%
\subsubsection{2.3.1 Psychometric measures}\label{psychometric-measures}}

The online survey contains a number of questionnaires: General psychological well-being is assessed using the German version of the WHO-5 scale.\textsuperscript{25,26}
To capture the construct of resilience, the German version 10-item-form of the Connor-Davidson resilience Scale (CD-RISC)\textsuperscript{29} is used.
Dispositional use of ER is assessed using the German version of the Emotion Regulation Questionnaire (ERQ).\textsuperscript{6,30}
For the assessment of ER ability we use the Flexible Emotion Regulation Scale (FlexER).\textsuperscript{31}
Implicit theories of willpower in emotion control are assessed using the implicit theories questionnaire from.\textsuperscript{32}
To assess Need for Cognition, the German version short form of the Need for Cognition Scale\textsuperscript{33,34} is used.
To assess self-control, sum scores of the German versions of the following questionnaires are used:\textsuperscript{35} the Self-Regulation Scale (SRS),\textsuperscript{36} the Brief Self-Control Scale (BSCS);\textsuperscript{37}{]},\textsuperscript{38} and the Barratt Impulsiveness Scale (BIS-11).\textsuperscript{39,40}
Attentional control is assessed using the Attentional Control Scale (ACS).\textsuperscript{41}
...
Pictures used in the paradigm were selected from the Emotional Picture Set (EmoPicS)\textsuperscript{42} and the International Affective Picture System (IAPS).\textsuperscript{43}
...
An evolutionary algorithm\textsuperscript{44} was used to cluster these pictures into five sets with comparable valence and arousal values (set one: V: \emph{M} \(\pm\) \emph{SD} = 2.84 \(\pm\) 0.57, A: \emph{M} \(\pm\) \emph{SD} = 5.62 \(\pm\) 0.34; set two: V: \emph{M} \(\pm\) \emph{SD} = 2.64 \(\pm\) 0.46, A: \emph{M} \(\pm\) \emph{SD} = 5.58 \(\pm\) 0.35; set three: V: \emph{M} \(\pm\) \emph{SD} = 2.82 \(\pm\) 0.62, A: \emph{M} \(\pm\) \emph{SD} = 5.60 \(\pm\) 0.39; set four: V: \emph{M} \(\pm\) \emph{SD} = 2.65 \(\pm\) 0.75, A: \emph{M} \(\pm\) \emph{SD} = 5.61 \(\pm\) 0.41; set five: V: \emph{M} \(\pm\) \emph{SD} = 2.74 \(\pm\) 0.70, A: \emph{M} \(\pm\) \emph{SD} = 5.63 \(\pm\) 0.37).
...
Participants will be healthy adults in the age of 18 to 30, recruited using the software \emph{ORSEE}\textsuperscript{22} at the Technische Universität Dresden.
...
Sample size calculation is done using \emph{G*Power}.\textsuperscript{45,46}
...
To explore the association between subjective arousal, physiological arousal, and subjective effort on SVs, a multilevel model (MLM) will be specified using the \(lmerTest\) package.\textsuperscript{47}
...
\] Level-1-predictors will be centered within cluster.\textsuperscript{48}
...
For each result of the analyses both, \(p\)-values and Bayes factor \(BF10\), calculated using the \emph{BayesFactor} package,\textsuperscript{49} will be reported. Bayses factors are calculated using the default prior widths of the functions \emph{anovaBF}, \emph{lmBF} and \emph{regressionBF}.
