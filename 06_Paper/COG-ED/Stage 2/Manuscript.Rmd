...
author: 
  ...
  - name          : "Alexander Strobel"
    ...
    role: 
      ...
      - Writing - review & editing
affiliation:
  - id            : "1" <!-- CK: im Prinzip kann diese Fußnote weggelassen werden, da wir alle die gleich Affilliation haben -->
    institution   : "Faculty of Psychology, Technische Universität Dresden, 01069 Dresden, Germany" <!-- CK: Postfach ist 01062 Dresden -->
authornote: |
abstract: |
  When individuals set goals, they consider the subjective value (SV) of the anticipated reward and the required effort, <!-- CK: "-" statt "," --> a trade-off that is of great interest to psychological research.
  One approach to quantify the SVs of levels of a cognitive task <!-- CK: Was meint hier levels? --> is the Cognitive Effort Discounting Paradigm by Westbrook and colleagues (2013).
  However, it fails to acknowledge the highly subjective nature of effort, as it assumes a <!-- CK: "an" --> unidirectional, inverse relationship between task load and SVs.
  ...
  We replicated the analysis of Westbrook and colleagues with our adaptation, the Cognitive and Affective Discounting (CAD) Paradigm, which quantifies SVs without assuming that the easiest level is preferred, thereby enabling the quantification of SVs for tasks without objective order of task load. <!-- CK: Ich würde hier zwei Sätze draus machen und den Punkt nach Paradigm setzen. Dadurch bekommt der erste Satz also die Entwicklung der Task mehr Gewicht. Als Vorschlag statt "our adaptation": "...with an adapted version of their paragigm, the ..." -->
  Results show that many participants preferred a more or the most difficult level.
...
We used *R* with *R Studio*[@RCT2020; @RStudioTeam2020] <!-- CK: die Refernz für "R" würde ich an "R" setzen --> with the main packages *afex*[@Singmann2021] and *BayesFactor*[@Morey2021] for all our analyses.
...
Participants received 24€ <!-- CK: €24, auch für die nachfolgenden Geldbeträge :) --> in total or course credit for participation.
...
Adding those same pairs again, but with the opposite assignment of fixed and flexible level, does approach the true preference, but has two disadvantages. <!-- CK: hier fehlt das Subjekt. it? -->
...
For each pair, the level that was chosen by the participant at least two out of three times will be used as the level with a flexible value, which starts at 1\ € and is changed <!-- CK: "changes"  -->in every iteration.
...
INSERT FIGURE 1 HERE <!-- CK: das kann raus, oder? -->
...
Several other personality questionnaires were used in this study but are the topic of the Registered Report for the second lab session[@ScheffelZerna2022]. <!-- CK: ich würde an dieser Stelle (oder gleich im zweiten Satz) kurz auflisten, welche anderen FB zum Einsatz kamen entsprechend des Simmons Statements -->
...
The paradigm started with the $n$-back levels one to four, presented sequentially with two runs per level, <!-- CK: and --> consisting of 64 consonants (16 targets, 48 non-targets) per run.
The levels were referred to by color (1-back black, 2-back red, 3-back blue, 4-back green <!-- CK: nach back würde ich ":" setzen oder vieleicht auch "-" -->) to avoid anchor effects in the effort discounting procedure.
...
They were told that one of their choices would be randomly picked for the final run of $n$-back, the data of which <!-- CK: ".However, the data was not analyzed ..." --> was not analyzed as it only served to incentivise truthful behavior and stay close to the design of Westbrook et al.[-@Westbrook2013].
...
Sample size determination was mainly based on the results of the analyses of Westbrook et al.[-@Westbrook2013] (see [Design Table](#DesignTableSection)). <!-- CK: ...in Supplementary Material -->
...
Second, we estimated a random slopes model of $n$-back level (level 1, fixed, and random factor: 0-back, 1-back, 2-back, 3-back <!-- CK: der random factor ist nicht mehr enthalten -->) predicting SV nested within subjects.
As Mussel et al.[-@Mussel2016] could show, participants with high versus low NFC not only have a more shallow decline in performance with higher $n$-back levels, but show a demand-specific increase in EEG theta oscillations, which has been associated with mental effort. <!-- CK: der Zusammenhang der Aussage mit dem Rest wird nicht klar -->
...
If necessary, we will adjusted the optimization algorithm to improve model fit.<!-- CK: anpassen an was gemacht wurde --> 
We visually inspected the residuals of the model. <!-- CK: zum Model criticism: das ist nicht mehr up to date und sollte so nicht gemacht werden. bitte auch in den Analysen anpassen /rausnehmen -->
...
<!-- CK: hier müssten noch eure Anpassungen mit rein: Kontraste etc., rounds?  -->
...
Due to a technical error the subjective task load data of one participant was incomplete, so the hypotheses involving the NASA Task Load Index <!-- CK: TLX --> were analyzed with $n=14$ data sets.
...
The LMM <!-- CK: Konsitenz: LMM oder MLM -->revealed $n$-back level as a reliable predictor of SV, even after controlling for declining task performance ($d'$ and median RT).
...
And secondly, we needed to re-assign the SCA to the MLM in hypothesis 2b, to which it belonged in the initial submission.
Following the advice of the reviewers, we had separated the NFC analysis from the MLM, and wanted to apply the SCA to the new NFC analysis (hypothesis 3a), as this was one of our main points of interest.
However, since hypothesis 3a does not contain any RT data and the SCA is therefore only useful for the MLM in hypothesis 2b, we applied it to the MLM.
...
The declining logistic contrast had the highest effect estimate (`r apa_print(emmeans::contrast(hypothesis2a_emm, method = list("Declining Linear" = c(3,1,-1,-3),"Ascending Quadratic" = c(-1,1,1,-1),"Declining Logistic" = c(3,2,-2,-3),"Positively Skewed Normal" = c(1,2,-1,-2))))$statistic$DecliningLogistic`), <!-- CK: stimmt irgendwie nciht mit der Table 2 überein -->suggesting a shallow decline of SVs between 1- and 2-back, and 3- and 4-back, respectively, and a steeper decline of SVs between 2- and 3-back.

Consequently, we had to adapt the multi level model <!-- CK: "MLM" --> to incorporate this non-linear trend.
...
<!-- CK: ok, ich sehe gerade, dass hier die Änderungen erklärt werden. -->
...
We did not use an optimization algorithm to improve the fit of the random intercept model. <!-- CK: ist das ncoh so und warum nicht? -->
A total of `r nrow(excl_cases_h2b)` data points from `r length(unique(excl_cases_h2b$subject))` participants were excluded, because the residuals exceeded 3 SD above the mean. <!-- CK: wie gesagt, das sollte noch angepasst bzw. rausgenommen werden. Vielleicht könnten wir so etwas schreiben wie: "We did not perform model criticism because we now believe that model-compliant exclusion of data is not a good scientific practice." --> 
...
To investigate the dependency of the model results on the RT preprocessing, we conducted a specification curve analysis (Figure\ \@ref(fig:sca-plot)). <!-- CK: ich dachte, das wäre raus und die SCA wäre nur für H3? -->
...
```{r nfc-sv, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Subjective values per n-back level. Each dot indicates a participant, the colours indicate their Need for Cognition (NFC) score. $N=116$. There is a horizontal jitter of 0.4 and a vertical jitter of 0.05 for visual clarity.", fig.pos="H"} 
...
Post-hoc tests showed that the difference between the SVs of 2- and 3-back is slightly more negative than the difference between 1- and 2-back (`r apa_print(pairs(hypothesis3a_emm_nlevel))$statistic`), but there were large inter-individual differences, especially for 2- and 3-back (left panel of Figure\ \@ref(fig:h3ac-plot)).
...
```{r h3ac-plot, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Difference scores for subjective values (left) and aversiveness ratings (right) when subtracting 2- from 1-back and 3- from 2-back. Each line indicates a participant, the colours indicate their Need for Cognition (NFC) score. $N=116$.", fig.pos="H"} 
...
```{r h3b-plot, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="NASA-TLX sum scores for each $n$-back level. Colours indicate Need for Cognition (NFC) score above or below the median. $N=116$.", fig.pos="H"} 
...
Post-hoc tests revealed that participants with NFC scores below the median reported higher aversiveness than participants with NFC scores above the median (`r apa_print(pairs(hypothesis3c_emm_nfc))$statistic`) (right panel of Figure\ \@ref(fig:h3ac-plot)).
Regardless of NFC, the difference of the aversiveness scores of 2- and 3-back was smaller and more negative than that of 1- and 2-back (`r apa_print(pairs(hypothesis3c_emm_levels))$statistic`), but again, there were large inter-individual differences.
...
This Registered Report aimed to adapt the Cognitive Effort Discounting (COG-ED) paradigm by Westbrook et al.[-@Westbrook2013], which can estimate <!-- CK: which estimates or estimating --> subjective values of different $n$-back levels, into the Cognitive and Affective Discounting (CAD) paradigm, which can estimate <!-- CK: s.o. --> SVs of tasks without defaulting to the objective task load as a benchmark.
For this purpose, we changed the way in which the discounting options are presented to the participants, based the anchor on their own choices, and computed SVs across multiple combinations of task levels.
...
Firstly, RT as a valid group-level indicator of performance might only be useful for levels up to $n=3$, and could be used to investigate inter-individual differences for $n>3$. <!-- CK: Warum für n größer 3? -->
...
This would support the notion of the $n$-back task offering a continuous manipulation of task load, as <!-- CK: "at"? --> least subjectively.
...
However, the declining logistic curve is similar to the sigmoidal curve that had the best fit in a different effort paradigm[@KleinFluegge2015], which the authors explained with the low effect of low energy costs, suggesting there are still common features of effort across different tasks and domains.
...
Despite the visual separation of the SVs of participants with very low and high NFC scores in higher $n$-back levels, the NFC group did not reach significance in predicting SVs.
...
<!-- CK: für die Diskussion wäre es noch gut, eine Entscheidung zu treffen, ob die jeweilige Hypothese angenommen oder abgelehnt wurde und das klar hinzuschreiben. -->
...
For instance, the differences between participants with higher and lower NFC should be investigated with extreme groups rather than a median split, especially in academic samples where NFC can be expected to be higher on average and more narrow in range.
...
Future studies could create a stronger separation between the concepts investigated <!-- CK: welche Konzepte sind das? -->in this study, and model the SVs and their task-related influencing factors first, before looking at (non-linear) associations with personality.
...
Nevertheless, we exceeded the largest required sample size by `r round(nrow(data_quest)/53, digits = 1)` times, which gives our analyses high statistical power, and we adhered to the agreed upon analyses of Stage 1.

## Conclusion

Effort and reward are two highly subjective concepts with relevance in daily life.
...
Therefore, we developed a paradigm that allows effort discounting independent of objective task load, which we tested using an $n$-back task.
The results showed that many participants <!-- CK: 34.5 % würde ich nicht als "many" verstehen --> indeed preferred a more or even the most difficult $n$-back level.
