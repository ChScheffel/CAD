...
The model had an effect size of `r $f^{2} = round(h2b_level_f2, digits = 2)$` for the *n*-back levels and `r $f^{2} = round(h2b_dprime_f2, digits = 2)$` for *d'*, which are considered large and small, respectively [@Cohen1992].
...
The effect size of the median RT was `r $f^{2} = format(round(h2b_medianRT_f2, digits = 2), nsmall = 2)$`.
...
Post-hoc tests showed that the difference between the SVs of 2- and 3-back is slightly more negative than the difference between 1- and 2-back (`r apa_print(pairs(hypothesis3a_emm_nlevel))$full_result`), but there are large interindividual differences, especially for 2- and 3-back (Figure\ \@ref(fig:h3a-plot)).
...
The rmANOVA on the association between NFC scores and NASA-TLX scores revealed a main effect of *n*-back level (`r apa_print(hypothesis3b_model)$full_result$level`) and an interaction between *n*-back level and NFC scores (`r apa_print(hypothesis3a_model)$full_result$nfcmedian_level`), but no main effect of NFC scores (`r apa_print(hypothesis3a_model)$full_result$nfcmedian`).
Post-hoc tests showed that the participants with NFC scores below the median had higher NASA-TLX scores for 3-back (`r apa_print(pairs(hypothesis3b_emm_interact))$full_result$X3_High_low`) and for 4-back (`r apa_print(pairs(hypothesis3b_emm_interact))$full_result$X4_High_low`) than those with NFC scores above the median.
...
apa_table(
  hypothesis2a_contrasts,
  caption = "Different contrasts for the rmANOVA comparing subjective values between *n*-back levels",
  note = "$SE$ = standard error, $df$ = degrees of freedom, $t$ = $t$-statistic, $p$ = $p$-value, CI = confidence interval.",
  escape = FALSE,
  placement = "H"
)
...
kable(h2b_result.table,
    ...
    footnote(general = "NFC = Need for Cognition, SE = standard error. ***$p$ < .001, **$p$ < .01, *$p$ < 0.5.",
             ...
             threeparttable = T)            # line breaks in the footnote
```
The intraclass correlation equals `r format(icc_h2b, digits = 2)`. 
...
## Hypothesis 3a: Participants with high NFC scores have higher subjective values for 2- and 3-back but lower subjective values for 1-back than participants with low NFC scores.

ANOVA:

```{r, echo=FALSE, message=FALSE, warning=FALSE} 
apa_table(
  hypothesis3a_rmanova,
  caption = "Main effects and interaction of NFC group and *n*-back level on subjective values",
  note = "NFC = Need for Cognition, Sum Sq = sum of squares, $df$ = degrees of freedom, $F$ = $F$-statistic, $p$ = $p$-value, CI = confidence interval.",
  escape = FALSE,
  placement = "H",
...
## Hypothesis 3b: Participants with high NFC scores have lower NASA-TLX scores in every *n*-back level than participants with low NFC scores.

ANOVA:

Main effect of the NFC group:

`r apa_print(hypothesis3b_model)$full_result$nfcmedian`, $BF10=$ `r format(extractBF(hypothesis3b_BF["nfcmedian"])$bf, digits = 2)`

Main effect of the *n*-back level:

`r apa_print(hypothesis3b_model)$full_result$level`, $BF10=$ `r format(extractBF(hypothesis3b_BF["level"])$bf, digits = 2)`

Interaction effect of NFC group and *n*-back level:

`r apa_print(hypothesis3b_model)$full_result$nfcmedian_level`, $BF10=$ `r format(extractBF(hypothesis3b_BF["level + nfcmedian"])$bf, digits = 2)`

Paired contrasts for the main effect of *n*-back level:

```{r, echo=FALSE, message=FALSE, warning=FALSE} 
apa_table(
  hypothesis3b_rmanova,
  caption = "Main effects and interaction of NFC group and *n*-back level on NASA-TLX scores",
  note = "NFC = Need for Cognition, Sum Sq = sum of squares, $df$ = degrees of freedom, $F$ = $F$-statistic, $p$ = $p$-value, CI = confidence interval.",
  escape = FALSE,
  placement = "H",
  font_size = "footnotesize")
