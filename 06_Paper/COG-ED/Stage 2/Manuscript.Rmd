...
affiliation:
  - id            : "1"
    institution   : "Faculty of Psychology, Dresden University of Technology, 01062 Dresden, Germany"
  - id            : "2"
    institution   : "Faculty of Psychology, University of Vienna, Vienna 1010, Austria"
authornote        :
abstract          : |
...
We used *R* (Version 4.2.0)[@RCT2020] with *R Studio* (Version 2022.12.0)[@RStudioTeam2020] with the main packages *papaja* (Version 0.1.1)[@Aust2020], *afex* (Version 1.2-1)[@Singmann2021], and *BayesFactor* (Version 0.9.12-4.4)[@Morey2021] for all our analyses.
...
However, this data was not analyzed as it only served to incentivise truthful behavior and to stay close to the design of Westbrook et al.[-@Westbrook2013].
...
Sample size determination was mainly based on the results of the analyses of Westbrook et al.[-@Westbrook2013] (see Design Table in the Supplementary Material).
...
To ensure the validity of this association, we conducted a specification curve analysis[@Simonsohn2020], which included 63 possible preprocessing pipelines of the RT data.
...
The Stage 1 Registered Report protocol has been approved and is available at [osf.io/cpxth/](https://osf.io/cpxth).
...
A fellow researcher made us aware that by using the z-transformed hit and false alarm rates for the computation of $d'$, the mean of $d'$ would be approximately 0 for each $n$-back level by design.
Consequently, $d'$ could not show changes across $n$-back levels in the manipulation check and would likely yield different results in the MLM.
Therefore, we computed $d'$ with unstandardized hit and false alarm rates.
We would like to thank G.C. for pointing us to this fallacy and included her as a co-author.

## Sample

Data was collected between the 16th of August 2022 and the 3rd of February 2023.
Of the $N=`r n_quest`$ participants who filled out the NFC questionnaire, $n=`r length(datalist_nback)`$ completed the first lab session.
...
Our final data set therefore included $N=`r nrow(data_quest)`$ participants (`r round((table(data_quest$gender)[2]/nrow(data_quest))*100, digits = 1)`% female, $M \pm SD=`r format(round(mean(data_quest[,"age"]), digits = 1), nsmall = 1)` \pm `r round(sd(data_quest[,"age"]), digits = 1)`$)\ years old), which is `r format(round(nrow(data_quest)/53, digits = 1), nsmall = 1)` times more than what the highest sample size calculation required.
...
In line with hypothesis H1a, the performance measure $d'$ decreased across $n$-back levels (`r apa_print(hypothesis1a_rmanova)$full_result`, `r apa_print(hypothesis1a_BF)$statistic`).
It decreased more strongly from 2- to 3-back (`r apa_print(pairs(hypothesis1a_emm))$statistic$X2_X3`, `r apa_print(BayesFactor::ttestBF(x = h1a_data$dprime[h1a_data$level == 2], y = h1a_data$dprime[h1a_data$level == 3], progress = FALSE, paired = TRUE))$statistic`) than from 1- to 2-back (`r apa_print(pairs(hypothesis1a_emm))$statistic$X1_X2`, `r apa_print(BayesFactor::ttestBF(x = h1a_data$dprime[h1a_data$level == 1], y = h1a_data$dprime[h1a_data$level == 2], progress = FALSE, paired = TRUE))$statistic`) or 3- to 4-back (`r apa_print(pairs(hypothesis1a_emm))$statistic$X3_X4`, `r apa_print(BayesFactor::ttestBF(x = h1a_data$dprime[h1a_data$level == 3], y = h1a_data$dprime[h1a_data$level == 4], progress = FALSE, paired = TRUE))$statistic`).
Similarly, the median RT increased across $n$-back levels (`r apa_print(hypothesis1b_rmanova)$full_result$level`, `r apa_print(hypothesis1b_BF)$statistic`), supporting hypothesis H1b.
...
Ratings on the physical subscale were higher for more difficult levels (`r apa_print(hypothesis1c_physical_rmanova)$full_result$level`, `r apa_print(hypothesis1c_physical_BF)$statistic`), apart from the contrasts 2- versus 3-back (`r apa_print(BayesFactor::ttestBF(x = h1c_data$physical[h1c_data$level == 2], y = h1c_data$physical[h1c_data$level == 3], progress = FALSE, paired = TRUE))$statistic`) and 3- versus 4-back (`r apa_print(BayesFactor::ttestBF(x = h1c_data$physical[h1c_data$level == 3], y = h1c_data$physical[h1c_data$level == 4], progress = FALSE, paired = TRUE))$statistic`).
...
The different curves of SVs across $n$-back levels can be seen in Figure\ \@ref(fig:sv-by-pref), grouped into those participants who had an SV of 1.0 for 1-back ($n=`r nrow(data_SV[data_SV$level==1 & data_SV$sv==1,])`$), for 2-back ($n=`r nrow(data_SV[data_SV$level==2 & data_SV$sv==1,])`$), for 3-back ($n=`r nrow(data_SV[data_SV$level==3 & data_SV$sv==1,])`$), for 4-back ($n=`r nrow(data_SV[data_SV$level==4 & data_SV$sv==1,])`$), or all SVs below 1.0, i.e. no absolute preference for any level ($n=`r length(setdiff(unique(data_SV$subject), data_SV$subject[data_SV$sv == 1]))`$).
...
Figure \@ref(fig:motivation) shows the different motivations in the context of the SVs per $n$-back level.

[FIGURE \@ref(fig:motivation) HERE]

The rmANOVA showed a significant difference between the SVs across $n$-back levels (`r apa_print(hypothesis2a_rmanova)$full_result$level`, `r apa_print(hypothesis2a_BF)$statistic`), so evidence was in favour of H2a.
...
The ICC of the null model indicated that there was a correlation of $r=$ `r sub("^(-?)0.", "\\1.", sprintf("%.3f", round(model0_h2b_ICC, digits = 3)))` between the SVs of a subject, i.e. that `r paste0(formatC(model0_h2b_ICC * 100, format = "f", digits = 2), "%")` of variance in SVs could be explained by differences between participants.
...
The final model had an effect size of $f^{2}=$ `r round(h2b_level_f2, digits = 2)` for the $n$-back levels and $f^{2}=$ `r round(h2b_dprime_f2, digits = 2)` for $d'$, which are considered medium and small, respectively [@Cohen1992].
...
The pipelines that yielded the highest estimates for $d'$ and the median RT used log-transformed data, irrespective of the dimension and exclusion criteria.
...
There was a main effect of the $n$-back level (`r apa_print(hypothesis3a_model)$full_result$nlevels`, `r apa_print(hypothesis3a_BF_nlevel)$statistic$nlevels`), but neither a main effect of the NFC group (`r apa_print(hypothesis3a_model)$full_result$nfcmedian`, `r apa_print(hypothesis3a_BF_nlevel)$statistic$nfcmedian`) nor an interaction of NFC group and $n$-back level (`r apa_print(hypothesis3a_model)$full_result$nfcmedian_nlevels`), so evidence was not in favour of H3a.
Post-hoc tests showed that the difference between the SVs of 2- and 3-back is slightly more negative than the difference between 1- and 2-back (`r apa_print(pairs(hypothesis3a_emm_nlevel))$statistic`), but there were large inter-individual differences (Supplementary Figure S2a).
...
Post-hoc tests showed that the participants with NFC scores below the median had higher NASA-TLX scores for 3-back (`r apa_print(pairs(hypothesis3b_emm_interact))$statistic$X3_High_low`, `r apa_print(BayesFactor::ttestBF(x = h3b_data$ntlx[h3b_data$nfcmedian == "low" & h3b_data$level == 3], y = h3b_data$ntlx[h3b_data$nfcmedian == "high" & h3b_data$level == 3], progress = FALSE, paired = FALSE))$statistic`) and for 4-back (`r apa_print(pairs(hypothesis3b_emm_interact))$statistic$X4_High_low`, `r apa_print(BayesFactor::ttestBF(x = h3b_data$ntlx[h3b_data$nfcmedian == "low" & h3b_data$level == 4], y = h3b_data$ntlx[h3b_data$nfcmedian == "high" & h3b_data$level == 4], progress = FALSE, paired = FALSE))$statistic`) than those with NFC scores above the median, so evidence was in favour of H3b.
...
There was a main effect of NFC group (`r apa_print(hypothesis3c_model)$full_result$nfcmedian`, `r apa_print(hypothesis3c_BF_levels)$statistic$nfc`) and a main effect of the $n$-back level (`r apa_print(hypothesis3c_model)$full_result$nlevels`, `r apa_print(hypothesis3c_BF_levels)$statistic$levels`), but no interaction (`r apa_print(hypothesis3c_model)$full_result$nfcmedian_nlevels`).
In favour of H3c, post-hoc tests revealed that participants with NFC scores below the median reported higher aversiveness than participants with NFC scores above the median (`r apa_print(pairs(hypothesis3c_emm_nfc))$statistic`) (Supplementary Figure S2b).
Regardless of NFC, the difference of the aversiveness scores of 2- and 3-back was more negative than that of 1- and 2-back (`r apa_print(pairs(hypothesis3c_emm_levels))$statistic`), indicating that in the same way in which the SVs decreased more strongly from 2- to 3-back than from 1- to 2-back, the aversion increased more strongly.
...
There was no main effect of NFC group (`r apa_print(explor_model)$full_result$nfcmedian`, `r apa_print(explor_BF)$table[2,"statistic"]`), but a main effect of the $n$-back level (`r apa_print(explor_model)$full_result$level`, `r apa_print(explor_BF)$table[1,"statistic"]`) and an interaction (`r apa_print(explor_model)$full_result$nfcmedian_level`).
...
Post-hoc tests for the interaction effect showed that the NFC groups only had a significant difference in SVs for 4-back, where participants below the NFC median had lower scores (`r apa_print(pairs(explor_emm_interact))$full_result$X4_High_low`).
Despite not reaching significance, 1-back was the only level in which participants with NFC scores above the median seemed to have lower SVs than those with scores below the median (`r apa_print(pairs(explor_emm_interact))$full_result$X1_High_low`).
...
Both the NFC score (`r apa_print(explor2_model)$full_result$nfc`, `r apa_print(explor2_BF_nfc)$statistic`) and the $n$-back level (`r apa_print(explor2_model)$full_result$level`, `r apa_print(explor2_BF_level)$statistic`) showed significant main effects, as well as a significant interaction (`r apa_print(explor2_model)$full_result$nfc_level`, `r apa_print(explor2_BF_interact)$statistic`).
Analyzing the estimated marginal means of the linear trends for each $n$-back level indicated a significant difference between the slopes of 1-back and 4-back (`r apa_print(explor2_slopes$contrasts)$full_result$Level1_level4`), but not between any other two levels.
...
This Registered Report aimed to adapt the Cognitive Effort Discounting (COG-ED) paradigm by Westbrook et al.[-@Westbrook2013], which estimates subjective values of different $n$-back levels, into the Cognitive and Affective Discounting (CAD) paradigm to estimate SVs of tasks without defaulting to the assumed objective task load as a benchmark.
...
This study also applied the CAD paradigm to an emotion regulation task, the results of which are detailed in a second Registered Report[@ScheffelZerna2022].

## Manipulation checks

Both $d'$ and the median RT changed across $n$-back levels, indicating that there was an increase in objective task load.
The steepest decrease in $d'$ between levels was from 2- to 3-back, resembling the declining logistic curve of the SVs.
...
Furthermore, several participants said that they perceived 3-back as more difficult than 4-back because they found it is easier to remember chunks of stimuli when $n$ was an even number than when $n$ was an odd number.
This would support the notion that the manipulation of task load in an $n$-back task is not strictly continuous.
And lastly, the fact that neither accuracy nor speed is an informative performance measure by itself has been observed before[@Meule2017] and both show different associations with various measures of intelligence[@Jaeggi2010], suggesting that they should always be reported as separate indices.
...
The fact that the majority of participants preferred lower over higher effort, but a minority showed the opposite pattern, is in line with previous research on cognitive effort by Kool et al. (2010)[-@Kool2010].
...
Thomson and Oppenheimer[-@Thomson2022] argue that the different effort curves that have been observed for different tasks are likely due to the fact that we still understand quite little about how and why different manipulations of effort work.
For example, the $n$-back task is likely not a continuous manipulation of task load, as discussed above.
However, the declining logistic curve is similar to the sigmoidal curve that has been found for a physical[@KleinFluegge2015] and a cognitive effort paradigm[@Massar2019], suggesting there are common features of effort across different tasks and domains.
...
Participants might have deliberately or subconsciously used the feedback they received at the end of each round, i.e. twice per $n$-back level, as an anchor during the effort discounting.
...
The declining logistic $n$-back levels and $d'$ remained significant predictors of SVs throughout all 63 preprocessing pipelines in the specification curve analysis.
The effect estimates of the $n$-back level varied by about `r round(diff(range(sca_results$beta_n)), digits = 3)`, those of $d'$ by about `r round(diff(range(sca_results$beta_d)), digits = 3)`.
In contrast to this stood the variability of the median RT effect estimates (around `r round(diff(range(sca_results$beta_r)), digits = 3)`), which did not reach significance in any pipeline.
...
Surprisingly, the $d'$ betas appeared almost unaffected by the preprocessing pipeline, even though $d'$ was computed after the outlier exclusion.
...
The majority of participants (`r round(100*((nrow(data_SV[data_SV$level==1 & data_SV$sv==1,]))/nrow(data_quest)), digits = 1)`\ %) had a preference for 1-back over the other levels, but that also means that there were `r round(100*((nrow(data_SV[data_SV$level!=1 & data_SV$sv==1,]))/nrow(data_quest)), digits = 1)`\ % who had a preference for 2-, 3-, or 4-back, and `r round(100*((length(setdiff(unique(data_SV$subject), data_SV$subject[data_SV$sv == 1])))/nrow(data_quest)), digits = 1)`\ % who preferred no specific level over all others.
...
Conceptually, this was likely due to the partial but not full overlap between self-reported and behavioural effort investment, which has also been found for a demand selection task[@Strobel2020].
...
Our data supports the notion of a Nonlinear Interaction between Person and Situation that has also been described by Schmitt et al. (2013)[-@Schmitt2013] and Blum et al. (2018)[-@Blum2018] in the same-named NIPS model.
...
An investigation of this relationship using the COG-ED paradigm[-@Westbrook2013] had been encouraged by Strobel et al.[@Strobel2020] based on their findings on demand avoidance and cognitive effort investment.
With the CAD paradigm, the declining logistic contrast of SVs across levels resembles the ascending logistic curve of the NIPS model[@Schmitt2013;@Blum2018] and should be tested further in a setting with $n$-back levels exceeding $n=4$.
...
However, findings by Bialaszek et al.[-@Bialaszek2017] suggest that participants are actually more sensitive to effort when the reward is small.
Nevertheless, we exceeded the largest required sample size by `r round(nrow(data_quest)/53, digits = 1)` times, which gives our analyses high statistical power.
...
JZ, CS, GC, and CK did the formal analysis.
