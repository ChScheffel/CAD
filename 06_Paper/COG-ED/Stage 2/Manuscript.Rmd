...
The results of each analysis was assessed on the basis of both $p$-value and the Bayes factor $BF10$, calculated with the *BayesFactor* package[@Morey2021] using the default prior widths of the functions *anovaBF*, *lmBF* and *ttestBF*.
We considered a $BF10$ close to or above 3/10 as moderate/strong evidence for the alternative hypothesis, and a $BF10$ close to or below .33/.10 as moderate/strong evidence for the null hypothesis [@Wetzels2015].
...
Of the $N=`r n_quest`$ participants who filled out the online questionnaires, $n=`r length(datalist_nback)`$ came to the first lab session.
...
The median RT did not differ between 2-back and 4-back, and the median RT was higher for 3-back than for 4-back (Table\ \@ref(tab:H1b-contrasts)).
...
When asking participants what motivated their decisions in the effort discounting, `r round((table(data_quest$motivation)[1]/sum(table(data_quest$motivation)))*100, digits = 1)`% stated that they wanted to avoid boredom, `r round((table(data_quest$motivation)[2]/sum(table(data_quest$motivation)))*100, digits = 1)`% stated that they wanted a challenge, `r round((table(data_quest$motivation)[3]/sum(table(data_quest$motivation)))*100, digits = 1)`% stated that they wanted to avoid effort, and `r round((table(data_quest$motivation)[4]/sum(table(data_quest$motivation)))*100, digits = 1)`% stated that they wanted to relax.
The remaining `r round((table(data_quest$motivation)[5]/sum(table(data_quest$motivation)))*100, digits = 1)`% of participants used the free text field and provided reasons such as "I wanted a fair relation of effort and reward.", "I wanted the fun that I had in the more challenging levels.", "I wanted to maximize reward first and minimize effort second.", or "I did not want to perform poorly when I was being paid for it.".
...
The declining logistic contrast had the highest effect estimate (`r apa_print(emmeans::contrast(hypothesis2a_emm, method = list("decl_log" =c(3,2,-2,-3))))$full_result`), suggesting a shallow decline of SVs between 1- and 2-back, and 3- and 4-back, respectively, and a steeper decline of SVs between 2- and 3-back.
...
Centered variables cannot be turned into factors, so we entered the variables in their raw form, and factors cannot be used as random slopes, so the model is now defined as:
$$
...
To provide more than one observation per factor level, we used the two rounds per level per subject, rather than levels per subject.
The ICC of the null model indicated that there is a correlation of $r=$`r round(model0_h2b_ICC, digits = 3)` between the SVs of a subject, i.e. that `r paste0(formatC(model0_h2b_ICC * 100, format = "f", digits = 2), "%")` of variance in SVs can be explained by differences between participants.
...
The model had an effect size of $f^{2}=$`r round(h2b_level_f2, digits = 2)` for the *n*-back levels and $f^{2}=$`r round(h2b_dprime_f2, digits = 2)` for *d'*, which are considered large and small, respectively [@Cohen1992].
...
```{r sca-plot, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Results of the multi level model for each of the 63 preprocessing pipelines. The lower panel indicates the type of preprocessing, the upper panel shows the beta coefficient of each predictor and its p-value. The colourbar indicates the BF10. The pipelines are sorted in descending order of the magnitude of the n-back level beta.", fig.pos="H"} 
  ...
  include_graphics(here("06_Paper", "Stage 2", "Figures", "sca-plot.png"))
...
The only pipelines, in which the median RT was a significant predictor of SVs, were the three pipelines with the highest Bayes Factors and the largest effect of *n*-back level.
...
The by far highest values of the beta of the median RT in Figure\ \@ref(fig:sca-plot) are all from the pipelines that used inverse transformation.
...
<!--
```{r nfc-sv, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Subjective values per n-back level. Each dot indicates a participant, the colours indicate their Need for Cognition score. $N=116$. There is a horizontal jitter of 0.4 and a vertical jitter of 0.05for visual clarity.", fig.pos="H"} 
  
  include_graphics(here("06_Paper", "Stage 2", "Figures", "nfc-sv.png"))
...
Post-hoc tests showed that the difference between the SVs of 2- and 3-back is slightly more negative than the difference between 1- and 2-back (`r apa_print(pairs(hypothesis3a_emm_nlevel))$full_result`), but there were large interindividual differences, especially for 2- and 3-back (Figure\ \@ref(fig:h3a-plot)).
...
<!--
```{r h3a-plot, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Difference scores in the subjective values of 1- minus 2-back and of 2- minus 3-back.", fig.pos="H"} 
  
  include_graphics(here("06_Paper", "Stage 2", "Figures", "h3a-plot.png"))

```
-->

The rmANOVA on the association between NFC scores and NASA-TLX scores revealed a main effect of *n*-back level (`r apa_print(hypothesis3b_model)$full_result$level`, $BF10=$`r round(BayesFactor::extractBF(hypothesis3b_BF_levels)$bf[1], digits = 2)`) and an interaction between *n*-back level and NFC scores (`r apa_print(hypothesis3b_model)$full_result$nfcmedian_level`), but no main effect of NFC scores (`r apa_print(hypothesis3b_model)$full_result$nfcmedian`, $BF10=$`r round(BayesFactor::extractBF(hypothesis3b_BF_levels)$bf[2], digits = 2)`).
Post-hoc tests showed that the participants with NFC scores below the median had higher NASA-TLX scores for 3-back (`r apa_print(pairs(hypothesis3b_emm_interact))$full_result$X3_High_low`, `r apa_print(BayesFactor::ttestBF(x = h3b_data$ntlx[h3b_data$nfcmedian == "low" & h3b_data$level == 3], y = h3b_data$ntlx[h3b_data$nfcmedian == "high" & h3b_data$level == 3], progress = FALSE, paired = FALSE))$statistic`) and for 4-back (`r apa_print(pairs(hypothesis3b_emm_interact))$full_result$X4_High_low`, `r apa_print(BayesFactor::ttestBF(x = h3b_data$ntlx[h3b_data$nfcmedian == "low" & h3b_data$level == 4], y = h3b_data$ntlx[h3b_data$nfcmedian == "high" & h3b_data$level == 4], progress = FALSE, paired = FALSE))$statistic`) than those with NFC scores above the median.
...
There was a main effect of NFC group (`r apa_print(hypothesis3c_model)$full_result$nfcmedian`, $BF10=$`r round(BayesFactor::extractBF(hypothesis3a_BF_nlevel)$bf[2], digits = 2)`) and a main effect of the *n*-back level (`r apa_print(hypothesis3c_model)$full_result$nlevels`, $BF10=$`r round(BayesFactor::extractBF(hypothesis3a_BF_nlevel)$bf[1], digits = 2)`), but no interaction.
Post-hoc tests revealed that participants with NFC scores below the median reported higher aversiveness than participants with NFC scores above the median (`r apa_print(pairs(hypothesis3c_emm_nfc))$full_result`).
Regardless of NFC, the difference of the aversiveness scores of 2- and 3-back was smaller and more negative than that of 1- and 2-back (`r apa_print(pairs(hypothesis3c_emm_levels))$full_result`), but again, there were large interindividual differences.
...
The MLM with the logistic contrast showed that the *n*-back level explained the majority of variance in SVs, while the performance measure *d'* also explained some variance in SVs, albeit small.
In both cases, the SVs decreased when the value of the predictor increased.
...
This feedback was based on correct responses and not on RT, so if participants based their effort discounting choices at least partly on this feedback, they were either motivated to repeat a task in which they perform well and/or they were reluctant to accept a larger reward for a task in which they did not perform well.
