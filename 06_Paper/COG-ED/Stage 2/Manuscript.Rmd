...
The hypothesis that yielded the largest necessary sample size was a repeated measures, within-factors ANOVA of reaction time and $n$-back level.
Sample size analysis with *G\*Power*[@Faul2007; @Faul2009] indicated that we should collect data from at least 53 participants, assuming $\alpha=.05$ and $\beta=.95$.
...
Our final data set therefore included $N=`r nrow(data_quest)`$ participants, which is `r round(nrow(data_quest)/53, digits = 1)` times more than what the highest sample size calculation required.
...
Post-hoc tests showed that the difference between the SVs of 2- and 3-back is slightly more negative than the difference between 1- and 2-back (`r apa_print(pairs(hypothesis3a_emm_nlevel))$statistic`), but there were large inter-individual differences, especially for 2- and 3-back (Figure\ \@ref(fig:h3a-plot)).
...
```{r h3a-plot, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Difference scores for subjective values when subtracting 2- from 1-back and 3- from 2-back. Each line indicates a participant, the colours indicate their Need for Cognition score. $N=116$.", fig.pos="H"} 
