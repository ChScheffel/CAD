...
First, the SVs are still quite alike among levels due to the fact that every more difficult level has only been compared with the easiest level, and second, having more task levels would lead to an exponential increase in comparisons.
...
Then, they complete the effort discounting procedure on screen and are instructed to do so as realistically as possible, even though the displayed rewards will not be paid out on top of their compensation.
They are told that one of their choices will be randomly picked for the final run of *n*-back, the data of which will not be analyzed as it serves to incentivise truthful behavior.
The procedure starts with a presentation of each possible pair of *n*-back levels, and the participants click on the level they would rather do for a reward of 1\ €.
Each pair is presented three times in total, with the order of presentation and the matter of which level is presented left/right fully randomized between participants.
After these 18 decisions, the paradigm assigns which level is flexible or fixed within each pair, based on which level was chosen at least twice or at the maximum once, respectively.
Then the effort discounting sensu Westbrook [@Westbrook2013] begins, with the pairs being presented in randomized order.
Each pair goes through all six iteration steps before moving on to the next pair.
After the computer paradigm, participants will fill out a short questionnaire on the tablet, indicating whether they have adhered to the instructions (yes/no) and what the primary motivation for their decisions during the effort discounting procedure was (avoid boredom/relax/avoid effort/seek challenge/other).
