...
And the last regression used the AUC of each participant's SV, the AUC of the NASA-TLX mental load subscale, and the AUC of the NASA-TLX effort subscale to predict each participant's NFC score.
...
```{r Hypothesis_3c, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  # H3c: The NASA-TLX dimensions effort and mental load predict individual NCS scores.

  # compute multiple linear regression to predict NCS based on SV AUC, Effort AUC and Mental Load AUC
  
  hypothesis3c <- summary(lm(h3_data$nfc ~ h3_data$auc_sv + h3_data$auc_mental + h3_data$auc_effort))
