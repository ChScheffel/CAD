...
**something is wrong with the AUC calculation**
...
demo_sv <- data.frame(subject = character(), auc_sv = double(), auc_sv_unsorted = double(),
                      ascent_mean = double(), ascent_sum = double(), auc_x_ascentmean = double())
for (i in 1:(length(subjectindex)-1)) {
  ...
  newdata <- data.frame(subject = demo_data$subject[subjectindex[i]],
                        auc_sv = auc(demo_data$level[subjectindex[i]:(subjectindex[i]+3)], demo_data$sv[subjectindex[i]:(subjectindex[i]+3)],
                                   method = "trapezoid"),
                        auc_sv_unsorted = (0.5 * (demo_data$sv[subjectindex[i]] + demo_data$sv[subjectindex[i]+1]) +
                                               0.5 * (demo_data$sv[subjectindex[i]+1] + demo_data$sv[subjectindex[i]+2]) +
                                               0.5 * (demo_data$sv[subjectindex[i]+2] + demo_data$sv[subjectindex[i]+3])),
...
ggplot(demo_data[c((1*4+1):(1*4+4),(4*4+1):(4*4+4),(15*4+1):(15*4+4),(17*4+1):(17*4+4)),], aes(x=level, y=sv, group=subject)) +
  geom_line(aes(color=subject), size = 1.5, position = position_jitter(w = 0.03, h = 0.03))
ggplot(demo_data[c((3*4+1):(3*4+4),(5*4+1):(5*4+4),(9*4+1):(9*4+4),(11*4+1):(11*4+4)),], aes(x=level, y=sv, group=subject)) +
  geom_line(aes(color=subject), size = 1.5, position = position_jitter(w = 0.03, h = 0.03))
ggplot(demo_data[c((0*4+1):(0*4+4),(2*4+1):(2*4+4),(21*4+1):(21*4+4),(23*4+1):(23*4+4)),], aes(x=level, y=sv, group=subject)) +
  geom_line(aes(color=subject), size = 1.5, position = position_jitter(w = 0, h = 0.03))
```
