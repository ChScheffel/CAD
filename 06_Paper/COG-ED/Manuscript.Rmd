...
Westbrook et al. [-@Westbrook2013] conceptualized NFC as a subjective self-report of ED for each participant which could then be related to the SVs as an objective measure.
...
Therefore, the solution lies in reducing the number of comparisons by presenting an ED round for each possible pair of levels, and by starting each round with a choice between equal prices.
...
Participants fill out the personality questionnaires online and then visit the lab for two sessions one week apart.
...
For each ED round, SVs are calculated by adding or subtracting 0.015625 from the last monetary value of the flexible level, depending on the participant's last choice.
Then, the final monetary values are divided by 2€, and the SV of each level per participant is computed by averaging all final values of each level, regardless of whether it was fixed or flexible.
...
However, RT increased with increasing n-back level (`r apa_print(hypothesis1b_rmanova)$full_result`).
...
The lack of difference between the SVs of 1- and 2-back indicates that when given the choice, not all participants preferred the easiest level.
