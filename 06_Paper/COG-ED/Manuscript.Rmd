...
```{r SCA_plot, echo=FALSE, message=FALSE, warning=FALSE, fig.show='hide'}
  ...
  sca_upperplot <- ggplot(data = sca_results, aes(x = num, y = beta, color = predictor, group = predictor)) +
    geom_point(data = sca_results, aes(x = num, y = sign, color = predictor, group = predictor)) +
    geom_line() +
    geom_text(aes(x = length(pipelines_data)-2, y = -0.01, label = "NFC")) + 
    geom_text(aes(x = length(pipelines_data)-2, y = -0.12, label = "Level")) +
    theme_classic() +
    labs(x = NULL, y = NULL) +
    scale_y_continuous(position = "right") +
    ggtitle("Fixed effects estimate") +
    theme(axis.text.x = element_blank(), plot.title = element_text(hjust = 1), legend.position = "none")
  ...
  sca_lowerplot <- ggplot(data = sca_results) +
    geom_point(aes(x = num, y = Dim, color = "red")) +
    geom_point(aes(x = num, y = Trans, color = "blue")) +
    geom_point(aes(x = num, y = Excl, color = "purple")) +
    theme_classic() +
    labs(x = NULL, y = NULL) +
    scale_y_reverse() +
