...
The R Markdown file used to analyze the data and write this document, as well as the raw data and the materials are freely available at github.com/ChScheffel/CERED.

## Questionnaires
To assess perceived task load, we used the 6-item NASA Task Load Index (NASA-TLX) [@Hart1988].
...
Responses to each item (e.g., "Thinking is not my idea of fun", recoded) were recorded on a 7-point Likert scale ranging from -3 (completely disagree) to +3 (completely agree).
...
The paradigm followed the same structure of task and ED procedure, but the emotion regulation strategy at the end was not randomly chosen, instaed participants could decide which strategy they wanted to use again.
...
Post-error trials were also excluded, because RT on those trials is longer due to more cautious behaviour [@Dutilh2012; @Houtman2012].
...
Another ANOVA with three linear contrasts was computed to compare the mean RT between two levels of 2-, 3-, and 4-back at a time.
...
```{r Hypothesis_1a, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  # FIXME:  actually, I guess, we have a rmANOVA here as we look at repeated measures of a subject's reaction time 
  #         under different load levels (= time points), see below
  
      # make a temporary copy of the dprime data frame without 1-back
      
      h1a_data <- dprime[dprime$level != 1,]      # FIXME: Westbrook et al. exclude level 1, do you understand why?
      h1a_data$level <- as.factor(h1a_data$level)
      
      # define contrasts
      
      contrasts(h1a_data$level) <- cbind(c(1,-1,0), c(0,1,-1), c(1,0,-1))     # there are predefined contrasts that may fit like contr.sum(3); contr.treatment(); contr.poly() 
      
      # calculate ANOVA
      
      hypothesis1a_anova <- summary(aov(h1a_data$d ~ h1a_data$level))         # this calculates ANOVA without the specified contrasts and might be redundant
      hypothesis1a_anova.lm <- summary.lm(aov(h1a_data$d ~ h1a_data$level))   # this gives you the contrasted result
      hypothesis1a_posthoc <- TukeyHSD(aov(h1a_data$d ~ h1a_data$level))      # post hoc analyses should be redundant as we already contrasted the levels
      
  # FIXME:  rmANOVA
      library(ez) # compute repeated measurement ANOVA
      h1a_data$subject <- factor(h1a_data$subject)
      # if there are reasonable concerns that we have NAs within our data set, we need to exclude this data beforehand 
      
      # https://stackoverflow.com/questions/35891062/planned-contrasts-using-ezanova-output-in-r
      # set orthogonal contrasts
      options(contrasts = c("contr.sum", "contr.poly"))
      hypothesis1a_rmanova <- ezANOVA(h1a_data, dv = d, wid = subject, within = level, type = 3, return_aov = T) 
      # I propose to use type-III sums of squares even if we do have a balanced design, but we may discuss that. 
      # alternatively we could use afex, results might be plotted easier; it is also possible to set up a linear model
      # TODO: find a way to plot means
        # if Mauchly's Test for Sphericity is sign > report Greenhouse-Geisser Epsilon (GGe) or Huynh-Feldt-Epsilon (HFe)
      
      # get emmeans
      library(emmeans)
      # this only works without the manually defined contrasts in line 439
      hypothesis1a_emm <- emmeans(hypothesis1a_rmanova$aov, ~ level) 
      # these means should be used for reporting/ plotting the means per load level
  
      # get the contrasts
      pairs(hypothesis1a_emm, adjust = "Holm")
      # or
      contrast(hypothesis1a_emm, list(c1 = c(1,-1,0), c2 = c(1,0,-1), c3 = c(0,1,-1)))
      
      # alternatively, post hoc tests can be run
      # as we have 3 levels in factor load level, we would run a fisher's least significant differences (FLSD)
      hypothesis1a_fisher <- ezStats(h1a_data, dv = d, wid = subject, within = level, type = 3)
      # calculate and save mean differences
      p1.p2 <- hypothesis1a_fisher$Mean[1] - hypothesis1a_fisher$Mean[2]
      p1.p3 <- hypothesis1a_fisher$Mean[1] - hypothesis1a_fisher$Mean[3]
      p2.p3 <- hypothesis1a_fisher$Mean[2] - hypothesis1a_fisher$Mean[3]
      
      #if absolute value is higher than fsld, the pairwise comparison is significant
      abs(p1.p2) > hypothesis1a_fisher$FLSD[1]
      abs(p1.p3) > hypothesis1a_fisher$FLSD[1]   
      abs(p2.p3) > hypothesis1a_fisher$FLSD[1]   
...
We first looked at how performance measures and NTLX ratings change as n-back levels increase.
Linear contrasts ANOVAs revealed decreasing performance *d'* with increasing n-back level (`r apa_print(hypothesis1a_anova)$full_result`).
Tukey's test confirmed that *d'* was smaller for *...(and put in row 1/2/3 ->)* (`r hypothesis1a_posthoc[["h1a_data$level"]][1,4]`).
...
# FIXME what about RT preprocessing?
  # correct / postcorrect     *see Westbrook et al. 2013, table 1: correct trials
  # log-transformation
  # valid answers: false alarm vs. missing -> plausible responses
...
Likewise, linear contrasts ANOVAs confirmed increases in RT with increasing n-back level (`r apa_print(hypothesis1b_anova)$full_result`).
Tukey's test showed that this was the case for *...(and put in row 1/2/3 ->)* (`r hypothesis1b_posthoc[["h1b_data$level"]][1,4]`).
...
```{r Hypothesis_1c, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  # six ANOVAs with six linear contrasts, contrasting the subscale scores of two n-back levels (1,2,3,4) at a time
      ...
      # TODO: see H1a. 
      num_vars <- c("mental", "physical", "time", "performance", "effort", "frustration")
      h1c_data[num_vars] <- lapply(h1c_data[num_vars], as.numeric)
      hypothesis1c_rmanova <- ezANOVA(h1c_data, dv = mental, wid = subject, within = level, 
                                       type = 3, return_aov = T)
      #...
      
      # We may also run a rmMANOVA
      # https://cran.r-project.org/web/packages/MANOVA.RM/vignettes/Introduction_to_MANOVA.RM.html
      library(MANOVA.RM)
      # we need to convert our data frame to clean wide or long format
...
Increases in subjective task load were investigated with linear contrast ANOVAs for each NTLX subscale, now including 1-back.
Perceived mental load increased with n-back (`r apa_print(hypothesis1c1_anova)$full_result`), especially from *...(and put in row 1/2/3/4/5/6 ->)* (`r hypothesis1c1_posthoc[["h1c_data$level"]][1,4]`).
Physical load increased as well, but not as strongly (`r apa_print(hypothesis1c2_anova)$full_result`), mainly from *...(and put in row 1/2/3/4/5/6 ->)* (`r hypothesis1c2_posthoc[["h1c_data$level"]][1,4]`).
...
# further preprocessing of nfc-data ---------------------------------------------
  # FIXME: actually we would now
  # 1. determine deviation from univariate normality using Shapiro Wilk test
  # 2. normalize and standardize sum scores by Blom's formula (1954)
  # 3. again determine deviation from univariate normality of data from second step using Shapiro Wilk test
  # this would mean:
  
  # required scripts (might be best at the beginning, following the packages) --
...
#determine deviation from univariate normality using Shapiro Wilk test
shapiro.summary = sw(data_nfc$nfc) 

#normalize and standardize sum scores by Blom's formula (1954)
data_nfc$nnfc = data.frame(normalize.gen(data_nfc$nfc))
colnames(data_nfc$nnfc) = "nfc.normal"

#determine deviation from univariate normality using Shapiro Wilk test
shapiro.summary = rbind(shapiro.summary, sw(data_nfc$nnfc))
...
# end --------------------------------------------------------------------------
  ...
  hypothesis3a.lm <- summary(lm(h3_data$nfc ~ h3_data$auc_sv))
  
  # FIXME: additionally, we could calculate the multiple correlation between both variables
  hypothesis3a.mcorr <- sqrt(hypothesis3a.lm$r.squared)
  # this correlation seems surprisingly high ...
