...
```{r preprocessing, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  # the data frame names are a combination of three letters from each of these categories, indicating how they will be preprocessed:
  #
  # RT transformations:
  #   (R) raw/none
  #   (L) log
  ...
  #   (N) not excluded
  #
  # Example: the data frame "RT_L3E" has log-transformed RT data with no data points beyond 3 SD from the mean and with no trials that followed misses/false alarms

  # one data frame without transformations is a simple copy of the original data frame
  RT_RNN <- data_nback

  # loop across participants and levels for log, inverse and square-root transform
  
      # make three more copies of the original data frame as a base
      RT_L <- data_nback
      RT_I <- data_nback
      RT_S <- data_nback
  ...
  # exclude trials after incorrect responses and with reaction times below 200 ms
  
  data_RT <- data_nback[-c(which(data_nback$correct == 1) + 1), ]
  data_RT <- data_RT[-c(which(data_RT$rt < 0.2)), ]
