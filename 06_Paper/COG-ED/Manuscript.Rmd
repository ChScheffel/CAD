...
```{r setup, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  # the following code does not work for me, gives an error message upon knitting:
  # > trying to use CRAN without setting a mirror
  # removing "papaja" from the list of packages (see below) does not solve the 
  # issue; perhaps we should setup the project using the renv package?
  ...
  # papaja must be installed from GitHub so code above will not work for this package
...
<!-- In the last sentence, the page number of the quote should appear with the quote, but given nature.csl does not, so perhaps just insert '(p. 209)' after 'reward sensitivity' -->  
...
Westbrook et al. [-@Westbrook2013] <!-- I guess, it is easier to simply write @Westbrook2013 instead of Westbrook et al. [-@Westbrook2013], but this may be a matter arising because of the use of the nature.csl --> used these SVs to investigate inter-individual differences in effort discounting (ED)<!-- perhaps avoid the abbreviation and simply always write effort discounting, we had some issues recently when using abbreviations in a manuscript, but we were using a lot of them, so it may be ok to use one or two here -->.
...
The individual degree of ED in the study by Westbrook et al. [-@Westbrook2013] was also associated with the participants' scores in Need for Cognition (NFC), a personality trait describing an individual's tendency to actively seek out and enjoy effortful cognitive activities [@Cacioppo1982].
...
With the present study, we change <!-- in my opinion better: alter --> one fundamental assumption of the original COG-ED paradigm: that the easiest *n*-back level has the highest SV.
We therfore adapted the COG-ED paradigm in such a way that it allows the computation of SVs for different *n*-back levels without presuming that all individuals inherently prefer the easiest level.
...
Therefore, the solution lies in reducing the number of necessary comparisons by presenting only one ED round for each possible pair of levels, and by starting each round with a choice between equal prices. <!--CK: instead of prices I would prefer reward in this context -->
...
```
<!--CK: Within the figure I recommend spaces, e.g. 2-back(1-back) > 2-back (1-back);
"rewarded" seems more appropriate to me than "priced";
the text in the small plots in the last line is pretty small and hard to read, is it possible to use a larger the font?
I understand the figure as you already explained it to me, but I suppose it would be beneficial to add some explanations, e.g.
to add two headings, i.e 1. calculation of the SVs (in different settings); 2. the resulting preference pattern
(and to add some explanations/ more details in the figure note) -->
...
Here we added <!-- why this 'here we added'? --> the hypothesis that (2c) SVs decline stronger with increasing task load for individuals with low compared to high NFC scores,
...
```{r Hypothesis2bc, echo=FALSE, message=FALSE, warning=FALSE, fig.show='hide'}
  ...
  h2b_r2 <- (h2b_total_r2[1,2] - h2b_without_r2[1,2]) / (1 - h2b_total_r2[1,2])
  ...
  # calculate f² with conditional R²
  h2c_r2 <- (h2b_total_r2[1,2] - h2c_without_r2[1,2]) / (1 - h2b_total_r2[1,2])
...
The study protocol complies with all relevant ethical regulations and was approved by the ethics committee of the Technische Universität Dresden (reference number EK...)<!--CK: don't forget to add -->. Prior to testing, written informed consent will be obtained. Participants will receive 30€ in total or course credit for participation.

## Pilot data

The sample of the pilot study consisted of $N=`r length(setindex)-1`$ participants (`r round((table(data_quest$gender)[2]/(length(setindex)-1))*100, digits = 1)`% female, $M=`r round(mean(data_quest[,"age"], na.rm = TRUE), digits = 1)`$ ($SD=`r round(sd(data_quest[,"age"], na.rm = TRUE), digits = 1)`$)\ years old). <!-- This results in 2 digits after the comma, all second digits are 0, perhaps use papaja::printnum instead of round, it will still be 2 digits, but with exact numbers --> 
...
Healthy participants aged 18 to 30\ <!-- welchen Effekt hat eigentlich der Backslash --> years will be recruited using the software *ORSEE* [@Greiner2015].
...
*A priori* sample size determination was performed using with *G\*Power* [@Faul2007; @Faul2009], mainly based on the results of the analyses of Westbrook et al. [-@Westbrook2013] (see Design Table<!-- link this table -->). Results of these analyses revealed that ... <!-- showed an increase in reaction time with higher *n*-back levels, indicated that we should collect data from at least 53 participants, assuming $\eta^2=0.04$, $\alpha=.05$, and $\beta=.95$. --> 
The power analyses of all other hypotheses yielded smaller necessary sample sizes.
To account for technical errors and exclusions of physiological data of the second lab session due to excessive noise, we aim to collect data of 80 participants.
...
For each ED round, SVs will be calculated by adding or subtracting 0.015625 <!--CK: for the reader it might be unclear how we determined this value -->from the last monetary value of the flexible level, depending on the participant's last choice.
...
First, we will calculate the intraclass correlation (ICC) as effect size measure for random effects. 
ICC displays the proportion of variance that is explained by differences between persons. <!--CS: Wieso wird der ICC nur für das Null Modell berechnet ? Man könnte den doch auch für das full-model ebenfalls angeben und sehen, wieviel Varianzaufklärung hinzugekommen ist? --> 
<!--CK: Die Intraklassenkorrelation wird immer auf Grundlage des Nullmodells berechnet. Sie gibt an, wieviel der Gesamtvarianz auf Unterschiede zwischen den Personen zurückgeführt werden kann. Das Nullmodell zeigt, wie stark sich die einzelnen Ebene-2-Einheiten in ihrer mittleren Ausprägung auf der aV unterscheiden. Daher enthält es keine Prädiktoren. Ich habe den letzten Teil des Satzes "on the basis of the null model" rausgenommen, da er redundant ist. -->
<!--CK: werden wie Korrelationskoeffizienten interpretiert (https://doi.org/10.1186/s40536-018-0061-2), könnten wir im Ergebnisteil demnach in der Größe einschätzen --->
...
As effect size measures, we calculate pseudo *R²* for our model and *f²* to estimate the effects of n-back level and NFC according to <!--CK: please add https://doi.org/10.1186/s40536-018-0061-2 (after Aiken and West 1991) -->.
<!--CK: these are the corresponding values: h2b_total_r2, h2b_r2, h2c_r2 -->
...
Since we do not have a fixed SV of 1 for 1-back, we cannot apply the "$AUC$" <!-- CK: it is the only case where "" are used -->computation of Westbrook et al. [-@Westbrook2013], which was the mean of the $AUCs$ of the SVs of each higher n-back level and 1-back, yielding values between 0\ and\ 1.
...
The results of each analysis will be assessed on the basis of both $p$-value and the Bayes factor $BF10$, calculated with the *BayesFactor* package [@Morey2021] using the default prior widths of the functions *anovaBF*, *lmBF* and *regressionBF*. <!-- hoffe, das reicht denen, denn wenn man genauer werden will, müsste man noch zwischen fixed und random effects priors unterscheiden, die zu allem Überdruß auch noch unterschiedliche Zahlenwerte haben, 1/2 für fixed und 1 für random bei anovaBF und lmBF, aber sqrt(2)/4 bei regressionBF -->
...
The paradigm code as well as the R Markdown file used to analyze the data and write this document is available at our [Github repository](https://github.com/ChScheffel/CERED). <!--CK: in the knitted document it is hard to see that "Github repository" contains a link, additionally it might be good to have the link visible within the document, also in case of a print version -->
...
Applications for the centralized funds will be reviewed in May. <!-- I would add the year -->
