...
```{r Hypothesis_3a, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  for (i in 1:(length(subjectindex)-1)) {
    ...
    newdata <- data.frame(subject = data_SV$subject[subjectindex[i]],
                          axauc = auc(data_SV$level[subjectindex[i]:(subjectindex[i]+3)], data_SV$sv[subjectindex[i]:(subjectindex[i]+3)],
                                               method = "trapezoid") * ((data_SV$sv[subjectindex[i]+3] - data_SV$sv[subjectindex[i]+2]) +
...
An increase in one AUC value corresponded, on average, to an increase of `r round(hypothesis3a$coefficients[2], digits = 1)` on the NFC scale (`r apa_print(hypothesis3a)$estimate$h3_data_axauc`).
