---
title             : "When easy is not preferred: The Cognitive and Emotion Regulation Effort Discounting paradigm (CERED) for estimating subjective values of cognitive tasks"
...
abstract: |
  Every day, individuals have to weigh the value of the reward when reaching a goal against the required effort.
  However, effort is not inherently aversive to all individuals, which has often been neglected by previous approaches to quantify the subjective value of effort.
  One of them is the Cognitive Effort Discounting paradigm by Westbrook et al. (2013), which can quantify the subjective values of the difficult levels of the n-back task against the easiest level.
  We aim to replicate the analysis of Westbrook et al. (2013) with our adaptation, the Cognitive and Emotion Regulation Effort Discounting paradigm (CERED).
  The CERED paradigm will likely enable a more insightful investigation of interindividual differences in effort discounting, as well as the investigation of effort discounting for task types for which this is currently not possible due to their high degree of subjectivity.
  
  We argue that adapting the paradigm and the method of computing the willingness to exert effort allows two crucial things: Quantifying subjective values without assuming that the easiest level is preferred, and quantifying subjective values for tasks with no objective order of task load.
...
```{r import, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  # remove trials with RT < 100 ms from the n-back data and exclude trials after incorrect trials
  data_RT <- data_nback[-c(which(data_nback$rt < 0.1)), ]
...
Regarding the associations of subjective and objective task load we hypothesize that (1a) performance measured by signal detection *d'* declines with increasing n-back level, (1b) performance measured by reaction time declines with increasing n-back level, and (1c) perceived task load increases with n-back level.
...
(2c) or (3a) or (3b) "SVs decline stronger with increasing load level for individuals with low compared to high NFC scores"
...
```{r preprocessing, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  #   (T) 0-100 ms after stimulus onset
  # Post-error trials:
  #   (E) excluded
  #   (N) not excluded
  #
  ...
  for (i in 1:length(pipelines_data)) {
    ...
    if (pipeline_name[[1]][1] == "A" & pipeline_name[[1]][2] == "A") {
          ...
          pipemedian <- median(pipelines_data[[i]]$rt, na.rm = TRUE)
          pipemad <- mad(pipelines_data[[i]]$rt, na.rm = TRUE)
          
          # the third decision is the outlier exclusion
      
          if (pipeline_name[[1]][4] == "2") {
            
            pipelines_data[[i]] <- pipelines_data[[i]][c(which(pipelines_data[[i]]$rt < (pipemedian + 2 * pipemad)
                                                               & pipelines_data[[i]]$rt > (pipemedian - 2 * pipemad))), ]
                
          } else if (pipeline_name[[1]][4] == "3") {
            
            pipelines_data[[i]] <- pipelines_data[[i]][c(which(pipelines_data[[i]]$rt < (pipemedian + 3 * pipemad)
                                                               & pipelines_data[[i]]$rt > (pipemedian - 3 * pipemad))), ]
          }
        ...
        for (x in 1:4) {
            ...
            pipemedian <- median(pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x], na.rm = TRUE)
            pipemad <- mad(pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x], na.rm = TRUE)
        ...
        for (x in 1:4) {
          ...
          for (y in 1:length(table(pipelines_data[[i]]$subject))) {
            ...
            pipemedian <- median(pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x & pipelines_data[[i]]$subject == pipesubject], na.rm = TRUE)
            pipemad <- mad(pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x & pipelines_data[[i]]$subject == pipesubject], na.rm = TRUE)
      ...
      # apply the transformations and exclusions within subjects but across levels
        ...
        for (y in 1:length(table(pipelines_data[[i]]$subject))) {
            ...
            pipemedian <- median(pipelines_data[[i]]$rt[pipelines_data[[i]]$subject == pipesubject], na.rm = TRUE)
            pipemad <- mad(pipelines_data[[i]]$rt[pipelines_data[[i]]$subject == pipesubject], na.rm = TRUE)
    ...
    # the 100-ms-exclusion and the post-error-exclusion are done outside of the A/A-A/W-W/W-W/A-conditions because they are independent of dimension:
    
    # exclude RTs fast than 100 ms after stimulus onset if the fourth letter in the data frame name is a "T"
    # also making sure that there are even any trials below 100 ms, otherwise you get an error message
    
    if (pipeline_name[[1]][4] == "T") {
      
      ifelse(nrow(pipelines_data[[i]][-c(which(pipelines_data[[i]]$rt < 0.1)), ]) > 0,
             pipelines_data[[i]] <- pipelines_data[[i]][-c(which(pipelines_data[[i]]$rt < 0.1)), ],
             pipelines_data[[i]] <- pipelines_data[[i]])
    }
...
The paradigm was written and presented using *Psychopy* [@Peirce2019]. We used *R Studio* [@RCT2020; @RStudioTeam2020] with the main packages *tidyverse* [@Wickham2019] and *basetestR* [@Makowski2019] for all our analyses.
...
A full list of measures can be found at osf.io/vnj8x/.

## Procedure

Healthy participants aged 18 to 30 years will be recruited using the software *ORSEE* [@Greiner2015]. Participants will fill out the personality questionnaires online and then visit the lab for two sessions one week apart. In the first session, participants provide informed consent and demographic data before completing the computer-based paradigm. The paradigm starts with the n-back levels one to four, presented sequentially with two runs per level, consisting of 64 consonants (16 targets, 48 non-targets) per run. The levels are referred to by color (1-back black, 2-back red, 3-back blue, 4-back green) to avoid anchor effects in the ED procedure. After each level, participants fill out the NASA-TLX on a tablet. Then, they complete the ED procedure on screen, where each possible pairing of the four n-back levels is presented in a randomized order. Participants are instructed to decide as realistically as possible, because one of their choices from the last iteration steps will be randomly chosen for one final run of n-back. This is only done to incentivise truthful behavior in the ED procedure, so the n-back data of this part will not be analyzed. The second session consists of an emotion regulation task with negative pictures and the instruction to suppress facial reactions, detach cognitively from the picture content, and distract oneself, respectively. The paradigm follows the same structure of task and ED procedure, but participants can decide which strategy they want to reapply in the last block. Participants will receive 30€ in total or course credit for participation. Study data will be collected and managed using REDCap electronic data capture tools hosted at Technische Universität Dresden [@Harris2009; @Harris2019].
...
In the pilot study, the online questionnaires were completed after the two lab sessions, but we will change this as described above, so that the participants do not have to come in again to collect their money or course credit.

## Analysis

We aim to conduct all analysis as described in Westbrook et al. [-@Westbrook2013], but the level of detail was not always sufficient, so there might be deviations regarding data cleaning and degrees of freedom. The performance measure *d'* will be computed as the difference of the *z*-transformed hit rate and the *z*-transformed false alarm rate [@Macmillan1990]. Reaction time (RT) data will be trimmed by excluding all trials with responses faster than 100 ms, as the relevant cognitive processes cannot have been completed before [@Whelan2008; @Berger2021]. Error- and post-error trials will be excluded in repeated measures analyses of variance (rmANOVA) and controlled for in multi-level-model (MLM), because RT on the latter is longer due to more cautious behavior [@Dutilh2012; @Houtman2012].
To test our hypotheses, we perform a series of rmANOVAs and MLM with orthogonal sum-to-zero contrasts in order to meaningfully interpret results [@Singmann2017]. 
Declining performance will be investigated by calculating a rmANOVA with three paired contrasts comparing *d'* between two levels of 2-, 3-, and 4-back at a time.
...
A rmANOVA with six paired contrasts will be computed, comparing the SVs between two levels of 1-, 2-, 3-, and 4-back at a time.
...
We apply restricted maximum likelihood (REML) to fit the model.
First, we calculate the intraclass correlation (ICC) on the basis of the null model.
Second, we estimate a random slopes model of SV including n-back load level as level-1-predictor and, additionally, NFC as level-2-predictor. 
Within the model, we control for *d'*, RT, correct and post-correct trials.
$$
SV \sim level\ * NFC + d' + RT + correct + postcorrect + (level|subject)
$$
Level-1-predictors are centered within cluster, whereas the level-2-predictor is centered at the grand mean as recommended by Enders & Tofighi [-@Enders2007].
Third, we perform a simple slopes analysis with n-back level as predictor and NFC as moderator. 
# TODO: Johnson-Neyman plot

To ensure the validity of the multi-level model, we will conduct a specification curve analysis [@Simonsohn2020], which will include 92 possible preprocessing pipelines of the RT data.
These pipelines specify which transformation was applied (none, log, inverse, or square-root), which outliers were excluded (none, $2 SD$ from the mean, $3 SD$ from the mean, RTs below 100 ms), whether post-error trials were excluded or not, and across which dimensions the transformations and exclusions were applied (across/within subjects and across/within n-back levels).
The multi-level model will be run with each of the 92 pipelines, which will also include our main pipeline (untransformed data, exclusion of RTs below 100 ms, removal of post-error trials).
...
Consequently, we chose a different way of quantifying the individual degree of ED.
...
```{r Hypothesis_1b, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  for (i in 1:(length(levelindex)-1)) {
    ...
    medianrt <- median(data_RT$rt[which(data_RT$correct[c(levelindex[i]:(levelindex[i+1])-1)] == 1)
...
RTs were longer for 3- than 2-back (`r apa_print(hypothesis1b_contrasts)$statistic[1]`), but not different between 2- and 4-back or 3- and 4-back.
...
SVs were higher for 1- than 3-back (`r apa_print(hypothesis2a_contrasts)$statistic$X1_X3`), for 1- than 4-back (`r apa_print(hypothesis2a_contrasts)$statistic$X1_X4`), and for 2- than 4-back (`r apa_print(hypothesis2a_contrasts)$statistic$X2_X4`).
...
```{r Hypothesis_2b, echo=FALSE, message=FALSE, warning=FALSE, fig.show='hide'}
  ...
  agg_vars <- cbind(data_SV, dprime["d"])
  h2b_data <- merge(agg_vars, data_nback, by = c("subject", "level"))
  
  
  # prepare temporary nfc-data frame for loop to feed into
  ...
  data_nfc$subject[which(data_nfc$subject == "E12K08")] <- "E12K08_neu"
  
  
  h2b_data <- merge(h2b_data, data_nfc, by = "subject")
  
  # include new variable for post-correct trials
  h2b_data$postcorrect = rep(NA, length(h2b_data$correct))  # add empty column
  for (z in 1:(length(h2b_data$correct)-1)) {               # for all trials except the very last one
    if (h2b_data$correct[z] == 1) {                         # if correct, paste correct
      h2b_data$postcorrect[z+1] <- 1
    } else if (h2b_data$correct[z] == 0) {                  # if not correct, paste incorrect
      h2b_data$postcorrect[z+1] <- 0
    } else {
      h2b_data$postcorrect[z+1] <- NA                       # if empty, leave empty
    } 
  }
    
  # 512 trials, 8 blocks á 64 trials
  i <- 1
  while (i <= nrow(h2b_data)){
    for (j in 1:64) {
      h2b_data$trial <- seq(1, 64, 1)
    }
    i <- i + 64
  }
  
  h2b_data$postcorrect[h2b_data$trial == "1"] <- 1         # avoid transfer from previous block and subject (first trial of a block is always correct)
  
  # get subset for MLM
  
  h2b_sub <- droplevels(subset(h2b_data[c("subject", "level", "sv", "d", "correct", "postcorrect", "rt", "nfc")],
                               subset = !is.na(rt)
                               & rt > 0.1))
  ...
  # d'
  h2b_sub$d.mwc <- ave(h2b_sub$d, h2b_sub$subject, FUN = function(x) mean(x, na.rm = T))
  h2b_sub$d.cwc <- h2b_sub$d - h2b_sub$d.mwc
  
  # RT
  h2b_sub$rt.mwc <- ave(h2b_sub$rt, h2b_sub$subject, FUN = function(x) mean(x, na.rm = T))
  h2b_sub$rt.cwc <- h2b_sub$rt - h2b_sub$rt.mwc
  
  # level
  h2b_sub$level.mwc <- ave(h2b_sub$level, h2b_sub$subject, FUN = function(x) mean(x, na.rm = T))
  h2b_sub$level.cwc <- h2b_sub$level - h2b_sub$level.mwc
  
  # correct 
  h2b_sub$correct.mwc <- ave(h2b_sub$correct, h2b_sub$subject, FUN = function(x) mean(x, na.rm = T))
  h2b_sub$correct.cwc <- h2b_sub$correct - h2b_sub$correct.mwc

  # postcorrect
  h2b_sub$postcorrect.mwc <- ave(h2b_sub$postcorrect, h2b_sub$subject, FUN = function(x) mean(x, na.rm = T))
  h2b_sub$postcorrect.cwc <- h2b_sub$postcorrect - h2b_sub$postcorrect.mwc
  
  ## level 2 predictor
  # > CGM

  # NFC
  h2b_sub$nfc.cgm <- scale(h2b_sub$nfc, scale = F)
  ...
  ## maximal model ---- 
  m1_h2b <- lmer(sv ~ level.cwc * nfc.cgm + d.cwc + rt.cwc + correct.cwc + postcorrect.cwc + (level.cwc|subject), 
                 data = h2b_sub,
  ...
  library(sjPlot)
  plot_model(m1_h2b, type = "diag")
  
  
  # simple slopes analysis -----------------------------------------------------
  
  library(interactions)
  m1_h2b.ss <- sim_slopes(m1_h2b, pred = level.cwc, modx = nfc.cgm, centered = "none", 
                    cond.int = T,                                       # print conditional intercepts
                    johnson_neyman = T, jnplot = T, control.fdr = T)    # calculate Johnson Neyman intervals, 
  ...
  # prepare table
  h2b_result.table <- as.data.frame(cbind(row.names(m1_h2b.fixef), 
                                          m1_h2b.fixef[,c(1, 2, 5)]))
  ...
  h2b_result.table$Parameter[1:8] <- c("Intercept", "N-Back Level", "NFC", "D-Prime", "RT", "Correct", "Post-Correct", "Level x NFC")
  # FIXME: display control variables? Spelling lower/upper case
  ...
  h2b_result.table$`Random Effects (SD)`[3:8] <- paste0("")
  ...
  #FIXME delete leading zero for p-values
    
  # prepare SSA results for reporting ------------------------------------------
  
  m1_h2b.ss$slopes
  
  library(huxtable)
  h2b_ssa.table <- as_huxtable(m1_h2b.ss)
  h2b_ssa.table[1,] <- c("Value of NFC", "Slope of N-Back Level", "Conditional Intercept")
  h2b_ssa.table <- h2b_ssa.table[-2,]
  
  h2b_ssa.table[2:4,1] <- c("- 1 SD", "Mean", "+ 1 SD")  
 
  
  # create interaction plot
  h2b_ssa.plot <- interact_plot(m1_h2b, pred = level.cwc, modx = nfc.cgm, centered = "none",
                                x.label = "n back load level", y.label = "subjective value",
...
The multilevel model `r replication_table`

statistically significant effect of n-back load level `r apa_print(m1_h2b)$full_result$level_cwc`
controlled for performance and correct and post-correct answers


`r apa_print(m1_h2b)$full_result$nfc_cgm`
`r round((icc_h2b*100), digits = 0)` % of the total variance in subjective values can be explained by differences between subjects ($ICC =$ `r round(icc_h2b, digits = 2)`).


simple slopes analysis `r h2b_ssa.table`, `r h2b_ssa.plot`
statistically significant influence of n-back load level on subjective value, but no moderating effect of NFC
