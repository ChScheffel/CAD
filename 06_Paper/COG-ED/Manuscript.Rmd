...
One task that is often used to investigate effort is the n-back task. <!-- Satz war sehr lang, ich ihn aufgeteilt und etwas umgestellt -->
In this working memory task, a continuous stream of stimuli, e.g. letters, is presented on screen. <!-- zweiter Teilsatz -->
Participants indicate via button press whether the current stimulus is the same as *n* stimuli before, with *n* being the level of difficulty between one and six [@Mackworth1959]; and are asked to respond as quickly and as accurately as they can. <!-- dritter Teilsatz-->
It requires simultaneous storage of past stimuli and processing of new stimuli [@Jonides1997]. <!-- Teilsatz nach dem Komma wirkt etwas out of place: der erste Teil beschäftigt sich mit cognitiven Anforderungen an die Teilnehmenden, der zweite aber mit einem Auftrag an die Probanden; Deshalb habe ich den Satz aufgeteilt und den zweiten Teil mit dem ersten verbunden -->
...
However, reliability measures of the n-back task are mixed, presumably due to ceiling effects in performance in the easiest level, causing inconsistent associations of n-back performance and measures such as executive functioning and fluid intelligence [@Jaeggi2010]. <!-- Ist hier als Reliabilität die Korrelation zum Außenkriterium gemeint (EF und gf?); Falls nicht, wären doch fehlende Reliabilitäten und Korrelationen zwei verschiedene Punkte-->

A way to quantify the subjective cost of each n-back level has been developed by Westbrook et al. [-@Westbrook2013], called the Cognitive Effort Discounting Paradigm (COG-ED). <!-- gibt es keine Möglichkeit, die Quelle automatisiert pasten zu lassen? Sonst müsste man an der Stelle auch zuerst alle drei Autoren nennen, oder?  -->
First, the participants complete the n-back levels to familiarize themselves with the task. <!-- habe es mal allgemeiner gehalten, da wir selbst auch keine sechs level haben  -->
Then, level one is compared with all following levels by asking the participants to decide between receiving 2\$ for the more difficult level or 1\$ for level one. <!-- s. vorheriger Kommentar -->
...
The idea is to estimate subjective equivalence, i.e., both offers are equally preferred [@Westbrook2013]. 
...
Westbrook et al. [-@Westbrook2013] used these SVs to investigate inter-individual differences in effort discounting by plotting the SVs and computing an area under the curve (AUC). <!-- kann man das so sagen? Mit dem plotten? Vielleicht sollte man einfach nur sagen, die AUC wurde berechnet? -->
Younger participants showed lower effort discounting, i.e. they needed a lower monetary incentive for choosing the more difficult levels over level one. <!--Würde bei dieser Aussage eine Ebene höher gehen und dann also vom Effort Discounting sprechen -->

## Effort discounting and need for cognition 
<!-- denke eine Zwischenüberschrift könnte das noch etwas deutlicher unterteilen-->
The AUC in the study by Westbrook et al. [-@Westbrook2013] was also associated with the participants' Need for Cognition (NFC) score, a personality trait describing individuals who actively seek and enjoy effortful cognitive activities [@Cacioppo1982]. <!-- Eventuell könnte man als Anfang für diesen Abschnitt nochmal kurz NFC definieren (zweiter Teilsatz) und kurz sagen, womit das assoziiert ist... also vor allem mit Traits, die danach klingen, als könnte Effort Discounting eine Rolle spielen; Dann mit Westbrook in Verbindung bringen, aber vielleicht nicht von der AUC reden, sondern lieber wieder von Effort Discounting? -->
...
## The present study
<!-- auch hier eine Zwischenüberschrift um es etwas besser in Blöcke einzuteilen -->
...
In fact, our COG-ED adaptation allows the computations of SVs, even if the individual does not have a clear preference for one task level. <!-- vielleicht eher "Additionally, our COG-ED adaption... -->
...
The COG-ED paradigm has been applied to tasks with different domains before, showing that SVs across task domains correlate [@Crawford2021], but it has not been applied to tasks without objective task load so far. <!-- Wortwiederholung "apply" und der Satz klingt etwas sperrig... vielleicht 2 drauß machen? --> 
...
```{r Hypothesis_1b, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  # ANOVA with three linear contrasts, contrasting the mean rt of two n-back levels (2,3,4) at a time
      ...
      h2a_data <- rt[rt$level != 1,]
      h2a_data$level <- as.factor(h2a_data$level)
      
      # define contrasts
      
      contrasts(h2a_data$level) <- cbind(c(1,-1,0), c(0,1,-1), c(1,0,-1))
      
      # calculate ANOVA
      
      hypothesis2a_anova <- summary(aov(h2a_data$meanrt ~ h2a_data$level))
      hypothesis2a_posthoc <- TukeyHSD(aov(h2a_data$meanrt ~ h2a_data$level))
      remove(h2a_data)

```

```{r Hypothesis_1c, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}

  # H1c: Ratings on all NTLX dimensions increase with increasing n-back level.

  # aggregate NTLX data per participant per level
  ...
  # six ANOVAs with six linear contrasts, contrasting the subscale scores of two n-back levels (1,2,3,4) at a time
      ...
      h3a_data <- data_ntlx
      h3a_data$level <- as.factor(h3a_data$level)
      
      # define contrasts
      
      contrasts(h3a_data$level) <- cbind(c(1,-1,0,0), c(0,1,-1,0), c(0,0,1,-1),
                                         c(1,0,-1,0), c(1,0,0,-1), c(0,1,0,-1))
      ## CS: das sieht mit den Kontrasten usw. irgendwie etwas kompliziert aus... Kannst gern mal beim ESER Code die ANOVAS mit dem aov_ez Package anschauen, vielleicht erleichtert es       das Ganze
      # calculate ANOVAs
      
      hypothesis3a1_anova <- summary(aov(h3a_data$mental ~ h3a_data$level))
      hypothesis3a1_posthoc <- TukeyHSD(aov(h3a_data$mental ~ h3a_data$level))

      hypothesis3a2_anova <- summary(aov(h3a_data$physical ~ h3a_data$level))
      hypothesis3a2_posthoc <- TukeyHSD(aov(h3a_data$physical ~ h3a_data$level))

      hypothesis3a3_anova <- summary(aov(h3a_data$time ~ h3a_data$level))
      hypothesis3a3_posthoc <- TukeyHSD(aov(h3a_data$time ~ h3a_data$level))

      hypothesis3a4_anova <- summary(aov(h3a_data$performance ~ h3a_data$level))
      hypothesis3a4_posthoc <- TukeyHSD(aov(h3a_data$performance ~ h3a_data$level))

      hypothesis3a5_anova <- summary(aov(h3a_data$effort ~ h3a_data$level))
      hypothesis3a5_posthoc <- TukeyHSD(aov(h3a_data$effort ~ h3a_data$level))

      hypothesis3a6_anova <- summary(aov(h3a_data$frustration ~ h3a_data$level))
      hypothesis3a6_posthoc <- TukeyHSD(aov(h3a_data$frustration ~ h3a_data$level))
