---
title             : "When easy is not preferred: A paradigm for estimating subjective values of cognitive tasks"
...
```{r setup, include=FALSE, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  # we also have the word count add-in by Ben Marwick, which can be installed via
...
However, its reliability measures are mixed, presumably due to ceiling effects in performance in the easiest level [@Jaeggi2010].
...
Findings on NFC and n-back performance are heterogeneous, ranging from no association of NFC and working memory [@Fleischhauer 2010; @Gaertner2021; @Hill2013] to positive associations of NFC with attention allocation [@Enge2008] and brain flexibility [@He2019].
...
Figure 1 illustrates how different modifications of the COG-ED paradigm by Westbrook et al. [-@Westbrook2013] return SVs that do or do not reflect the true preference of a hypothetical participant, who likes 2-back most, 3-back less, and 1-back least.
...
Therefore, the solution lies in reducing the number of necessary comparisons by presenting an ED round for each possible pair of levels, and by starting each round with a choice between equal prices.
...
Additionally, we will compare the ED behaviour of participants regarding the n-back task and an emotion regulation task, the full results of the latter will be published in a second Registered Report.
The COG-ED paradigm has been applied to tasks with different domains before, showing that SVs across task domains correlate [@Crawford2021], but these tasks had an objective order of task load, which is not the case for emotion regulation.

## Hypotheses
Our hypotheses were derived from the results of Westbrook et al. [-@Westbrook2013].
Regarding the associations of subjective and objective task load we hypothesized that (1a) performance measured by signal detection *d'* declines with increasing n-back level, (1b) performance measured by reaction time declines with increasing n-back level, and (1c) perceived task load increases with n-back level.
...
```{r import_image, out.width="50%", warning=FALSE}

  knitr::include_graphics(here("99_Thinktank", "Other", "Paradigm_Scheme.png"))

```

## Procedure
Healthy participants aged 18 to 30 years will be recruited using the software *ORSEE* [@Greiner2015].
Each participant fills out the personality questionnaires online and then visit the lab for two sessions one week apart.
...
After each level, participants fill out the NASA-TLX on a tablet to provide a state measure of cognitive effort.
...
This is done to incentivise truthful behavior in the ED procedure, the n-back data of this part is not analyzed.
...
The paradigm follows the same structure of task and ED procedure, but the emotion regulation strategy at the end is not randomly chosen, instead participants can decide which strategy they want to use again.
Participants receive 30€ in total or course credit for participation.
...
The sample of the pilot study consisted of $N=$ `r length(datalist_ED)` participants (`r round((table(data_quest$gender)[2]/length(datalist_ED))*100, digits = 1)`% female, $M=$`r round(mean(data_quest$age, na.rm = TRUE), digits = 1)`($SD=$`r round(sd(data_quest$age, na.rm = TRUE), digits = 1)`) years old).

## Analysis
We aim to conduct all analysis as described in Westbrook et al. [-@Westbrook2013], but the level of detail was not always sufficient, so there might be deviations in terms of data cleaning and degrees of freedom.
The performance measure *d'* is computed as the difference of the *z*-transformed hit rate and the *z*-transformed false alarm rate [@Macmillan1990].
Reaction time (RT) data is trimmed by excluding all trials with responses faster than 200 ms, as the relevant cognitive processes cannot be completed before [@Whelan2008; @Berger2021].
Error- and post-error trials are excluded, because RT on the latter is longer due to more cautious behavior [@Dutilh2012; @Houtman2012].
Declining performance is investigated by calculating a repeated measures analysis of variance (rmANOVA) with three paired contrasts comparing *d'* between two levels of 2-, 3-, and 4-back at a time.
...
To investigate changes in NASA-TLX ratings, six rmANOVAs are computed, one for each NASA-TLX subscale, and each with six paired contrasts comparing the ratings between two levels of 1-, 2-, 3-, and 4-back at a time.
The SVs are calculated by adding or subtracting the final iteration value of 0.015625 from the monetary value of the flexible level for each ED round, depending on the participant's choice in the last iteration.
Then, the final monetary values are divided by 2€, and the SV of each level per participant is computed by averaging all final monetary values of each level, regardless of whether it was fixed or flexible.
A rmANOVA with six paired contrasts is computed, comparing the SVs between two levels of 1-, 2-, 3-, and 4-back at a time.
Tukey method is used for the paired contrasts of each rmANOVA, including *p*-value adjustment.

To determine the influence of task performance in the association of SVs and n-back level, we will set up a multi-level model.
**MLM explanation goes here**
To ensure the validity of the multi-level model, we conduct a specification curve analysis [@Simonsohn2020], which includes 92 possible preprocessing pipelines of the RT data.
These pipelines specify which transformation has been applied (none, log, inverse, or square-root), which outliers have been excluded (none, $2 SD$ from the mean, $3 SD$ from the mean, RTs below 200 ms), whether post-error trials have been excluded or not, and across which dimensions the transformations and exclusions have been applied (across/within subjects and across/within n-back levels).
The multi-level model will be run with each of the 92 pipelines, which also includes our main pipeline (untransformed data, exclusion of RTs below 200 ms, removal of post-error trials).
...
The association of ED and NFC is examined with a regression using the AUC of each participant's SVs to predict their NFC score.
A second regression additionally includes the mean of the NASA-TLX subscales' AUCs of each participant as a predictor.
...
Therefore, we multiply both indicators to arrive at a value reflecting both degree and direction of preference, called $AxAUC$.

##Power analysis
A sample size analysis with GPower [@Faul2007; @Faul2009], using the pilot RT data, indicated that we should collect data from at least 40 participants, assuming $f=0.27$, $\alpha=.05$, $\beta=.80$, and no correlation between levels.
...
As expected, RT increased with increasing n-back level (`r apa_print(hypothesis1b_rmanova)$full_result`).
RTs were longer for 3- than 2-back (`r apa_print(hypothesis1b_contrasts)$statistic[1]`), but not significantly different between 2- and 4-back or 3- and 4-back.
Even though *d'* did not decrease across n-back levels in this very small sample, the increases in RT indicate an increase in objective task load.
...
Ratings on the physical load and the performance subscales only increased between 1- and 3-back (`r apa_print(hypothesis1c_physical_contrasts)$statistic$X1_X3`, and `r `apa_print(hypothesis1c_performance_contrasts)$statistic$X1_X3`, respectively) and 1- and 4-back (`r apa_print(hypothesis1c_physical_contrasts)$statistic$X1_X4`, and `r `apa_print(hypothesis1c_performance_contrasts)$statistic$X1_X4`, respectively).
...
Overall, these ratings showed that the increase in objective task load was also being subjectively perceived by the participants.
...
SVs were higher for 1- than for 3-back (`r apa_print(hypothesis2a_contrasts)$statistic$X1_X3`), for 1- than for 4-back (`r apa_print(hypothesis2a_contrasts)$statistic$X1_X4`), and for 2- than for 4-back (`r apa_print(hypothesis2a_contrasts)$statistic$X2_X4`).
Since the SVs of 1- and 2-back were not significantly different, we can conclude that when given the choice, not all participants preferred the easiest n-back level.
...
An increase in one NASA_NTLX AUC value corresponded, on average, to a decrease of `r round(hypothesis3b$coefficients[2], digits = 1)` on the NFC scale (`r apa_print(hypothesis3b)$estimate$h3_data_auc_ntlx`).
