...
```{r setup, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  packages = c("bibtex", "here", "tidyverse", "bayestestR", "papaja", "lmerTest", "afex", "emmeans", "sjPlot",
               "kableExtra", "interactions", "glmmTMB", "BayesFactor", "ggplot2", "egg", "knitr", "effectsize")
...
Since we also aim to establish this paradigm for the assessment of tasks with no objective task load, e.g. emotion regulation tasks, we call it the Cognitive and Emotion Regulation Effort Discounting Paradigm (CERED).
...
The full results of the latter will be published in a second Registered Report.
...
```{r figure1, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="An example for subjective values for an n-back task with three levels, returned by different modifications of the COG-ED paradigm for a participant with the true preference 2-back > 3-back > 1-back.", fig.pos="H"} 
...
Regarding the associations of task load and ED we hypothesize that (2a) SVs decline with increasing n-back level, and (2b) they do so even after controlling for declining task performance.
Here we added the hypothesis that (2c) SVs decline stronger with increasing task load for individuals with low compared to high NFC scores.
...
```{r pipelinesdprime, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r Hypothesis1a, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  hypothesis1a_contrasts$etasq <- c(format(t_to_eta2(t = hypothesis1a_contrasts$t.ratio[1], df_error = hypothesis1a_contrasts$df[1], ci = 0.95), digits = 2)[1,1],
                                     format(t_to_eta2(t = hypothesis1a_contrasts$t.ratio[2], df_error = hypothesis1a_contrasts$df[2], ci = 0.95), digits = 2)[1,1],
                                     format(t_to_eta2(t = hypothesis1a_contrasts$t.ratio[3], df_error = hypothesis1a_contrasts$df[3], ci = 0.95), digits = 2)[1,1])
  
  hypothesis1a_contrasts$CI <- c(format(t_to_eta2(t = hypothesis1a_contrasts$t.ratio[1], df_error = hypothesis1a_contrasts$df[1], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1a_contrasts$t.ratio[2], df_error = hypothesis1a_contrasts$df[2], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1a_contrasts$t.ratio[3], df_error = hypothesis1a_contrasts$df[3], ci = 0.95), digits = 2)[1,2])
...
```{r Hypothesis1b, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  hypothesis1b_contrasts$etasq <- c(format(t_to_eta2(t = hypothesis1b_contrasts$t.ratio[1], df_error = hypothesis1b_contrasts$df[1], ci = 0.95), digits = 2)[1,1],
                                     format(t_to_eta2(t = hypothesis1b_contrasts$t.ratio[2], df_error = hypothesis1b_contrasts$df[2], ci = 0.95), digits = 2)[1,1],
                                     format(t_to_eta2(t = hypothesis1b_contrasts$t.ratio[3], df_error = hypothesis1b_contrasts$df[3], ci = 0.95), digits = 2)[1,1])
  
  hypothesis1b_contrasts$CI <- c(format(t_to_eta2(t = hypothesis1b_contrasts$t.ratio[1], df_error = hypothesis1b_contrasts$df[1], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1b_contrasts$t.ratio[2], df_error = hypothesis1b_contrasts$df[2], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1b_contrasts$t.ratio[3], df_error = hypothesis1b_contrasts$df[3], ci = 0.95), digits = 2)[1,2])
...
```{r Hypothesis1c, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  hypothesis1c_mental_contrasts$etasq <- c(format(t_to_eta2(t = hypothesis1c_mental_contrasts$t.ratio[1], df_error = hypothesis1c_mental_contrasts$df[1], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_mental_contrasts$t.ratio[2], df_error = hypothesis1c_mental_contrasts$df[2], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_mental_contrasts$t.ratio[3], df_error = hypothesis1c_mental_contrasts$df[3], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_mental_contrasts$t.ratio[4], df_error = hypothesis1c_mental_contrasts$df[4], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_mental_contrasts$t.ratio[5], df_error = hypothesis1c_mental_contrasts$df[5], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_mental_contrasts$t.ratio[6], df_error = hypothesis1c_mental_contrasts$df[6], ci = 0.95), digits = 2)[1,1])
  hypothesis1c_mental_contrasts$CI <- c(format(t_to_eta2(t = hypothesis1c_mental_contrasts$t.ratio[1], df_error = hypothesis1c_mental_contrasts$df[1], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_mental_contrasts$t.ratio[2], df_error = hypothesis1c_mental_contrasts$df[2], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_mental_contrasts$t.ratio[3], df_error = hypothesis1c_mental_contrasts$df[3], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_mental_contrasts$t.ratio[4], df_error = hypothesis1c_mental_contrasts$df[4], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_mental_contrasts$t.ratio[5], df_error = hypothesis1c_mental_contrasts$df[5], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_mental_contrasts$t.ratio[6], df_error = hypothesis1c_mental_contrasts$df[6], ci = 0.95), digits = 2)[1,2])
  
  hypothesis1c_physical_contrasts$etasq <- c(format(t_to_eta2(t = hypothesis1c_physical_contrasts$t.ratio[1], df_error = hypothesis1c_physical_contrasts$df[1], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_physical_contrasts$t.ratio[2], df_error = hypothesis1c_physical_contrasts$df[2], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_physical_contrasts$t.ratio[3], df_error = hypothesis1c_physical_contrasts$df[3], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_physical_contrasts$t.ratio[4], df_error = hypothesis1c_physical_contrasts$df[4], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_physical_contrasts$t.ratio[5], df_error = hypothesis1c_physical_contrasts$df[5], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_physical_contrasts$t.ratio[6], df_error = hypothesis1c_physical_contrasts$df[6], ci = 0.95), digits = 2)[1,1])
  hypothesis1c_physical_contrasts$CI <- c(format(t_to_eta2(t = hypothesis1c_physical_contrasts$t.ratio[1], df_error = hypothesis1c_physical_contrasts$df[1], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_physical_contrasts$t.ratio[2], df_error = hypothesis1c_physical_contrasts$df[2], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_physical_contrasts$t.ratio[3], df_error = hypothesis1c_physical_contrasts$df[3], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_physical_contrasts$t.ratio[4], df_error = hypothesis1c_physical_contrasts$df[4], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_physical_contrasts$t.ratio[5], df_error = hypothesis1c_physical_contrasts$df[5], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_physical_contrasts$t.ratio[6], df_error = hypothesis1c_physical_contrasts$df[6], ci = 0.95), digits = 2)[1,2])
  
  hypothesis1c_time_contrasts$etasq <- c(format(t_to_eta2(t = hypothesis1c_time_contrasts$t.ratio[1], df_error = hypothesis1c_time_contrasts$df[1], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_time_contrasts$t.ratio[2], df_error = hypothesis1c_time_contrasts$df[2], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_time_contrasts$t.ratio[3], df_error = hypothesis1c_time_contrasts$df[3], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_time_contrasts$t.ratio[4], df_error = hypothesis1c_time_contrasts$df[4], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_time_contrasts$t.ratio[5], df_error = hypothesis1c_time_contrasts$df[5], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_time_contrasts$t.ratio[6], df_error = hypothesis1c_time_contrasts$df[6], ci = 0.95), digits = 2)[1,1])
  hypothesis1c_time_contrasts$CI <- c(format(t_to_eta2(t = hypothesis1c_time_contrasts$t.ratio[1], df_error = hypothesis1c_time_contrasts$df[1], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_time_contrasts$t.ratio[2], df_error = hypothesis1c_time_contrasts$df[2], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_time_contrasts$t.ratio[3], df_error = hypothesis1c_time_contrasts$df[3], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_time_contrasts$t.ratio[4], df_error = hypothesis1c_time_contrasts$df[4], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_time_contrasts$t.ratio[5], df_error = hypothesis1c_time_contrasts$df[5], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_time_contrasts$t.ratio[6], df_error = hypothesis1c_time_contrasts$df[6], ci = 0.95), digits = 2)[1,2])
  
  hypothesis1c_effort_contrasts$etasq <- c(format(t_to_eta2(t = hypothesis1c_effort_contrasts$t.ratio[1], df_error = hypothesis1c_effort_contrasts$df[1], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_effort_contrasts$t.ratio[2], df_error = hypothesis1c_effort_contrasts$df[2], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_effort_contrasts$t.ratio[3], df_error = hypothesis1c_effort_contrasts$df[3], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_effort_contrasts$t.ratio[4], df_error = hypothesis1c_effort_contrasts$df[4], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_effort_contrasts$t.ratio[5], df_error = hypothesis1c_effort_contrasts$df[5], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_effort_contrasts$t.ratio[6], df_error = hypothesis1c_effort_contrasts$df[6], ci = 0.95), digits = 2)[1,1])
  hypothesis1c_effort_contrasts$CI <- c(format(t_to_eta2(t = hypothesis1c_effort_contrasts$t.ratio[1], df_error = hypothesis1c_effort_contrasts$df[1], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_effort_contrasts$t.ratio[2], df_error = hypothesis1c_effort_contrasts$df[2], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_effort_contrasts$t.ratio[3], df_error = hypothesis1c_effort_contrasts$df[3], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_effort_contrasts$t.ratio[4], df_error = hypothesis1c_effort_contrasts$df[4], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_effort_contrasts$t.ratio[5], df_error = hypothesis1c_effort_contrasts$df[5], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_effort_contrasts$t.ratio[6], df_error = hypothesis1c_effort_contrasts$df[6], ci = 0.95), digits = 2)[1,2])
  
  hypothesis1c_frustration_contrasts$etasq <- c(format(t_to_eta2(t = hypothesis1c_frustration_contrasts$t.ratio[1], df_error = hypothesis1c_frustration_contrasts$df[1], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_frustration_contrasts$t.ratio[2], df_error = hypothesis1c_frustration_contrasts$df[2], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_frustration_contrasts$t.ratio[3], df_error = hypothesis1c_frustration_contrasts$df[3], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_frustration_contrasts$t.ratio[4], df_error = hypothesis1c_frustration_contrasts$df[4], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_frustration_contrasts$t.ratio[5], df_error = hypothesis1c_frustration_contrasts$df[5], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_frustration_contrasts$t.ratio[6], df_error = hypothesis1c_frustration_contrasts$df[6], ci = 0.95), digits = 2)[1,1])
  hypothesis1c_frustration_contrasts$CI <- c(format(t_to_eta2(t = hypothesis1c_frustration_contrasts$t.ratio[1], df_error = hypothesis1c_frustration_contrasts$df[1], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_frustration_contrasts$t.ratio[2], df_error = hypothesis1c_frustration_contrasts$df[2], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_frustration_contrasts$t.ratio[3], df_error = hypothesis1c_frustration_contrasts$df[3], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_frustration_contrasts$t.ratio[4], df_error = hypothesis1c_frustration_contrasts$df[4], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_frustration_contrasts$t.ratio[5], df_error = hypothesis1c_frustration_contrasts$df[5], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_frustration_contrasts$t.ratio[6], df_error = hypothesis1c_frustration_contrasts$df[6], ci = 0.95), digits = 2)[1,2])
  
  hypothesis1c_performance_contrasts$etasq <- c(format(t_to_eta2(t = hypothesis1c_performance_contrasts$t.ratio[1], df_error = hypothesis1c_performance_contrasts$df[1], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_performance_contrasts$t.ratio[2], df_error = hypothesis1c_performance_contrasts$df[2], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_performance_contrasts$t.ratio[3], df_error = hypothesis1c_performance_contrasts$df[3], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_performance_contrasts$t.ratio[4], df_error = hypothesis1c_performance_contrasts$df[4], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_performance_contrasts$t.ratio[5], df_error = hypothesis1c_performance_contrasts$df[5], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis1c_performance_contrasts$t.ratio[6], df_error = hypothesis1c_performance_contrasts$df[6], ci = 0.95), digits = 2)[1,1])
  hypothesis1c_performance_contrasts$CI <- c(format(t_to_eta2(t = hypothesis1c_performance_contrasts$t.ratio[1], df_error = hypothesis1c_performance_contrasts$df[1], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_performance_contrasts$t.ratio[2], df_error = hypothesis1c_performance_contrasts$df[2], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_performance_contrasts$t.ratio[3], df_error = hypothesis1c_performance_contrasts$df[3], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_performance_contrasts$t.ratio[4], df_error = hypothesis1c_performance_contrasts$df[4], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_performance_contrasts$t.ratio[5], df_error = hypothesis1c_performance_contrasts$df[5], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis1c_performance_contrasts$t.ratio[6], df_error = hypothesis1c_performance_contrasts$df[6], ci = 0.95), digits = 2)[1,2])
...
```{r Hypothesis2a, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  hypothesis2a_contrasts$etasq <- c(format(t_to_eta2(t = hypothesis2a_contrasts$t.ratio[1], df_error = hypothesis2a_contrasts$df[1], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis2a_contrasts$t.ratio[2], df_error = hypothesis2a_contrasts$df[2], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis2a_contrasts$t.ratio[3], df_error = hypothesis2a_contrasts$df[3], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis2a_contrasts$t.ratio[4], df_error = hypothesis2a_contrasts$df[4], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis2a_contrasts$t.ratio[5], df_error = hypothesis2a_contrasts$df[5], ci = 0.95), digits = 2)[1,1],
                                    format(t_to_eta2(t = hypothesis2a_contrasts$t.ratio[6], df_error = hypothesis2a_contrasts$df[6], ci = 0.95), digits = 2)[1,1])
  hypothesis2a_contrasts$CI <- c(format(t_to_eta2(t = hypothesis2a_contrasts$t.ratio[1], df_error = hypothesis2a_contrasts$df[1], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis2a_contrasts$t.ratio[2], df_error = hypothesis2a_contrasts$df[2], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis2a_contrasts$t.ratio[3], df_error = hypothesis2a_contrasts$df[3], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis2a_contrasts$t.ratio[4], df_error = hypothesis2a_contrasts$df[4], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis2a_contrasts$t.ratio[5], df_error = hypothesis2a_contrasts$df[5], ci = 0.95), digits = 2)[1,2],
                                 format(t_to_eta2(t = hypothesis2a_contrasts$t.ratio[6], df_error = hypothesis2a_contrasts$df[6], ci = 0.95), digits = 2)[1,2])
...
```{r Hypothesis2bc, echo=FALSE, message=FALSE, warning=FALSE, fig.show='hide'}
...
```{r SCAplot, echo=FALSE, message=FALSE, warning=FALSE, fig.show='hide'}
  ...
  Figure3 <- ggarrange(sca_upperplot, sca_lowerplot, nrow = 2, heights = c(1,2.5))

```

```{r Hypothesis3a, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r Hypothesis3b, echo=FALSE, message=FALSE, warning=FALSE}
...
We used *R Studio* [@RCT2020; @RStudioTeam2020] with the main packages *afex* [@Singmann2021] and *bayestestR* [@Makowski2019] for all our analyses.

## Ethics information

## Pilot data

The sample of the pilot study consisted of $N=`r length(setindex)-1`$ participants (`r round((table(data_quest$gender)[2]/(length(setindex)-1))*100, digits = 1)`% female, $M=`r round(mean(data_quest[,"age"], na.rm = TRUE), digits = 1)`$ ($SD=`r round(sd(data_quest[,"age"], na.rm = TRUE), digits = 1)`$)\ years old).
...
The MLM revealed that n-back level was a reliable predictor of SV, even after controlling for declining task performance (d’ and RT) as well as correct and post-correct answers, while NFC was not.
The specification curve analysis showed that this pattern was true for all 63 pipelines.
And finally, while the $AxAUC$ value did not predict any amount of variance in individual NFC scores, the $AUC$ of NASA-TLX scores did.
...
A sample size analysis with _G*Power_ [@Faul2007; @Faul2009], based on the results of the ANOVA of Westbrook et al. [-@Westbrook2013] which showed an increase in reaction time with higher n-back levels, indicated that we should collect data from at least 53 participants, assuming $\eta2=0.04$, $\alpha=.05$, and $\beta=.95$.
...
To determine the influence of task performance in the association of SVs and n-back level, we will set up a MLM using the $lmerTest$ package [@Kuznetsova2017]. 
We will apply restricted maximum likelihood (REML) to fit the model.
First, we will calculate the intraclass correlation (ICC) on the basis of the null model. <!--CS: Wieso wird der ICC nur für das Null Modell berechnet ? Man könnte den doch auch für das full-model ebenfalls angeben und sehen, wieviel Varianzaufklärung hinzugekommen ist? -->
...
<!--CS: Man könnte noch die Effektgröße bestimmen nach: doi: 10.1002/sim.3429 -> semi partial R²  --> 
...
JZ and CS conceptualized the study and its methodology, acquired funding, investigated, administered the project, and wrote the software.
...
*Figure 1.* An example for subjective values for an n-back task with three levels, returned by different modifications of the COG-ED paradigm for a participant with the true preference 2-back > 3-back > 1-back.
...
`r paste("[", round(h2c_jn.int[1], digits = 2), ", ", round(h2c_jn.int[2], digits = 2), "]", sep = "")`
