...
Therefore, the solution lies in reducing the number of necessary comparisons by presenting only one effort discounting round for each possible pair of levels, and by starting each round with a choice between equal prices. <!--CK: instead of prices I would prefer reward in this context -->
...
For each effort discounting round, SVs will be calculated by adding or subtracting 0.015625 <!--CK: for the reader it might be unclear how we determined this value -->from the last monetary value of the flexible level, depending on the participant's last choice.
