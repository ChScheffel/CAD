...
```{r setup, include=FALSE, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  library(nlme)         # fitting nonlinear mixed-effects model
...
Post-error trials were also excluded, because RT on those trials is longer due to more cautious behavior [@Dutilh2012; @Houtman2012].
...
```{r Hypothesis_2b, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  # define variables -------------------------------------------------------------
  # dependent variable:     SV                    numeric                 data_SV
  # indepenendent variable: n-back level [1-4]    numeric                       
  # controlled regressors:  d'                    numeric                 dprime
  #                         mRT                   numeric                 rt
  ...
  # check and specify correct data types -----------------------------------------
  str(h2b_data)
  ...
  # change global coding to orthogonal sum-to-zero contrast by -------------------
  options(contrasts = c("contr.sum", "contr.poly"))
  
  
  # null model -------------------------------------------------------------------
  # m0_h2b <- nlme(model = sv ~ 1,
  #                data = h2b_data,
  #                random = ~ 1 | subject, 
  #                method = "REML")
  ...
  # "subject-specific intercepts" speaks for random-slopes-model, as within this model 
  # the slope is allowed to vary between subjects 
  
  # m1_h2b <- nlme(sv ~ d.cwc + meanrt.cwc + level.cwc + (level.cwc| subject),
  #                data = h2b_data,
  #                fixed = ...,
  #                random = ...,
  #                control = ...,
  ...
  # initVals <- getInitial(sv ~ d.cwc + meanrt.cwc + level.cwc, data = h2b_data)
  
  # get initial values for mlm start
  fm.lm <- lm(sv ~ d.cwc + meanrt.cwc + level.cwc, h2b_data)
  st <- coef(fm.lm)
  
  m1_h2b <- nlme(sv ~ d.cwc + meanrt.cwc + level.cwc,
                 data = h2b_data,
                 #fixed = d.cwc + meanrt.cwc + level.cwc ~ 1,
                 fixed = list(d.cwc ~ 1, meanrt ~ 1, level.cwc ~ 1),
                 random = level.cwc ~ 1 | subject,
                 start = list(fixed = st[2:4]),
                 method = "REML")
  
  
  # change global coding to default ----------------------------------------------
