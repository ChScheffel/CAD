...
We aim to conduct all analysis as described in Westbrook et al. [-@Westbrook2013], but the level of detail was not always sufficient, so there might be deviations regarding data cleaning and degrees of freedom. The performance measure *d'* will be computed as the difference of the *z*-transformed hit rate and the *z*-transformed false alarm rate [@Macmillan1990]. Reaction time (RT) data will be trimmed by excluding all trials with responses faster than 100 ms, as the relevant cognitive processes cannot have been completed before [@Whelan2008; @Berger2021]. Error- and post-error trials will be excluded, because RT on the latter is longer due to more cautious behavior [@Dutilh2012; @Houtman2012].
Declining performance will be investigated by calculating a repeated measures analysis of variance (rmANOVA) with three paired contrasts comparing *d'* between two levels of 2-, 3-, and 4-back at a time.
...
null model > ICC
random slopes model
  controlled for performance (d', RT), correct and post-correct trials
...
```{r Hypothesis_2b, echo=FALSE, message=FALSE, warning=FALSE, fig.show='hide'}
  ...
  m1_h2b.ss <- sim_slopes(m1_h2b, pred = level.cwc, modx = nfc.cgm, centered = "none", 
                    ...
                    johnson_neyman = T, jnplot = T, control.fdr = T)    # calculate Johnson Neyman intervals, 
                                                                        # create plot, adjust false discovery rate
  ...
  # TODO: Corinna go on here :) 
    # -> prepare tables: MLM results + huxtable
    # -> write methods
    # -> write results
