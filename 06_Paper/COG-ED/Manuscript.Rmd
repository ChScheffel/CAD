...
```{r Hypothesis_3a, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  data_hyp3 <- data.frame(subject = character(), auc = double())
  ...
  for (i in 1:(length(subjectindex)-1)) {
    ...
    newdata <- data.frame(subject = data_SV$subject[subjectindex[i]],
                         auc = auc(data_SV$level[subjectindex[i]:(subjectindex[i+1]-1)], data_SV$sv[subjectindex[i]:(subjectindex[i+1]-1)],
  ...
  hypothesis3a <- summary(lm(data_hyp3$nfc ~ data_hyp3$auc))
