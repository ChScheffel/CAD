...
**MLM explanation goes here** To ensure the validity of the multi-level model, we will conduct a specification curve analysis [@Simonsohn2020], which will include 92 possible preprocessing pipelines of the RT data.
...
```{r Hypothesis_2b, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  library(huxtable)
  as_huxtable(m1_h2b.ss)
  
  plot(m1_h2b.ss)
  
  interact_plot(m1_h2b, pred = level.cwc, modx = nfc.cgm, centered = "none",
                x.label = "n back load level", y.label = "subjective value", legend.main = "NFC", interval = T) + theme_apa()
  
  # FIXME: Do not print all charts
  
  # prepare results for reporting ----------------------------------------------
  
  names(summary(m1_h2b))
  ...
  h2b_result.table$`p-Value`[which(h2b_result.table$`p-Value`<.001)] <- 
    paste0(h2b_result.table$`p-Value`[which(h2b_result.table$`p-Value`<.001)],"*")
  ...
  # FIXME: display of p = 0.000,  no display of NA
  
  apa_table(h2b_result.table,
            caption = "Subjective values decline with increasing n-back level, even after controlling for declining task 
            performance (d’ and reaction time), correct and postcorrect trials.",
            note = "")
  ...
  # TODO: 
    # -> check whether we want to include an additional hypothesis
...
(`r apa_table(h2b_result.table)`)

intraclass correlation "icc_h2b"
