...
```{r setup, include=FALSE, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  library(snow)         # for faster processing
...
In the present study, we will validate the CERED paradigm by conceptually replicating the ED and NFC findings of Westbrook et al. [-@Westbrook2013].
...
However, we propose that (3c) when taking individual preferences for different n-back levels into account, the prediction of NFC scores by SVs can be improved significantly.
...
```{r import, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  data_quest <- read.csv(here("04_RawData", "Sampledata", "COGED_DATA.csv"), stringsAsFactors = FALSE, header = TRUE)
...
```{r preprocessing, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  #
  # across which dimensions the tranformation and outlier exclusion will be done:
  ...
  #   (N) none
  #   (2) 2 SDs from the mean
  #   (3) 3 SDs from the mean
  #   (T) 0-200 ms after stimulus onset
  # Post-error trials:
  #   (E) excluded
  #   (N) not excluded
  #
  # Example: the data frame "AWL3E" has log-transformed RT data with no data points beyond 3 SD
  # from the mean and with no trials that followed misses/false alarms
  #
  # Some combinations are nonsensical, so we will not include them: Raw data sets (__R___) with either no outlier
  ...
  pipelines_data <- replicate(92, data_nback, simplify = FALSE)
  pipelines_data <- setNames(pipelines_data, c("AARNN","AARNE","AAR2N","AAR2E","AAR3N","AAR3E","AARTN","AARTE",
                                               "AALNN","AALNE","AAL2N","AAL2E","AAL3N","AAL3E",
                                               "AAINN","AAINE","AAI2N","AAI2E","AAI3N","AAI3E",
                                               "AASNN","AASNE","AAS2N","AAS2E","AAS3N","AAS3E",
                                               "AWR2N","AWR2E","AWR3N","AWR3E",
                                               "AWLNN","AWLNE","AWL2N","AWL2E","AWL3N","AWL3E",
                                               "AWINN","AWINE","AWI2N","AWI2E","AWI3N","AWI3E",
                                               "AWSNN","AWSNE","AWS2N","AWS2E","AWS3N","AWS3E",
                                               "WWR2N","WWR2E","WWR3N","WWR3E",
                                               "WWLNN","WWLNE","WWL2N","WWL2E","WWL3N","WWL3E",
                                               "WWINN","WWINE","WWI2N","WWI2E","WWI3N","WWI3E",
                                               "WWSNN","WWSNE","WWS2N","WWS2E","WWS3N","WWS3E",
                                               "WAR2N","WAR2E","WAR3N","WAR3E",
                                               "WALNN","WALNE","WAL2N","WAL2E","WAL3N","WAL3E",
                                               "WAINN","WAINE","WAI2N","WAI2E","WAI3N","WAI3E",
                                               "WASNN","WASNE","WAS2N","WAS2E","WAS3N","WAS3E"))
  
  for (i in 1:length(pipeline_data)) {
    
    # extract the name of the current data frame
    
    pipeline_name <- strsplit(names(pipelines_data)[i], "")
    
    # replace the RTs of incorrect trials with NA for better handling
    
    pipelines_data[[i]]$rt[pipelines_data[[i]]$correct == 0] <- NA
    ...
    if (pipeline_name[[1]][1] == "A" & pipeline_name[[1]][2] == "A") {
          ...
          # the second decision is the type of transformation
          
          if (pipeline_name[[1]][3] == "L") {
            
            pipelines_data[[i]]$rt <- log10(pipelines_data[[i]]$rt)
            
          } else if (pipeline_name[[1]][3] == "I") {
            
            pipelines_data[[i]]$rt <- 1/(pipelines_data[[i]]$rt)
            
          } else if (pipeline_name[[1]][3] == "S") {
            
            pipelines_data[[i]]$rt <- sqrt(pipelines_data[[i]]$rt)
          }
      
          # compute mean and standard deviation
      
          pipemean <- mean(pipelines_data[[i]]$rt, na.rm = TRUE)
          pipesd <- sd(pipelines_data[[i]]$rt, na.rm = TRUE)
          
          # the third decision is the outlier exclusion
      
          if (pipeline_name[[1]][4] == "2") {
            
            pipelines_data[[i]] <- pipelines_data[[i]][c(which(pipelines_data[[i]]$rt < (pipemean + 2 * pipesd)
                                                               & pipelines_data[[i]]$rt > (pipemean - 2 * pipesd))), ]
                
          } else if (pipeline_name[[1]][4] == "3") {
            
            pipelines_data[[i]] <- pipelines_data[[i]][c(which(pipelines_data[[i]]$rt < (pipemean + 3 * pipesd)
                                                               & pipelines_data[[i]]$rt > (pipemean - 3 * pipesd))), ]
            
          } else if (pipeline_name[[1]][4] == "T") {
            
            pipelines_data[[i]] <- pipelines_data[[i]][-c(which(pipelines_data[[i]]$rt < 0.2)), ]
          }
  ...
  # one data frame without transformations is a simple copy of the original data frame
  # put that data frame in a list in which all data frames will be and replace the RTs of incorrect trials with NA

  pipelines_data <- list("RNN" = data_nback)
  pipelines_data[["RNN"]]$rt[pipelines_data[["RNN"]]$correct == 0] <- NA

  # add raw version without post-error trials

  pipelines_data[["RNE"]] <- pipelines_data[["RNN"]][-c(which(pipelines_data[["RNN"]]$correct == 0) + 1), ]
  
  # add raw versions with 0-200ms exclusion
  
  pipelines_data[["RTN"]] <- pipelines_data[["RNN"]][-c(which(pipelines_data[["RNN"]]$rt < 0.2)), ]
  pipelines_data[["RTE"]] <- pipelines_data[["RTN"]][-c(which(pipelines_data[["RTN"]]$correct == 0) + 1), ]

  # group 1: across subjects, across conditions
  
    # raw versions
    
    tempmean <- mean(pipelines_data[["RNN"]]$rt[pipelines_data[["RNN"]]$correct == 1])
    tempsd <- sd(pipelines_data[["RNN"]]$rt[pipelines_data[["RNN"]]$correct == 1])
    
    pipelines_data[["AAR2N"]] <- pipelines_data[["RNN"]][c(which(pipelines_data[["RNN"]]$rt < (tempmean+2*tempsd) & pipelines_data[["RNN"]]$rt > (tempmean-2*tempsd))), ]
    pipelines_data[["AAR2E"]] <- pipelines_data[["AAR2N"]][-c(which(pipelines_data[["AAR2N"]]$correct == 0) + 1), ]
    pipelines_data[["AAR3N"]] <- pipelines_data[["RNN"]][c(which(pipelines_data[["RNN"]]$rt < (tempmean+3*tempsd) & pipelines_data[["RNN"]]$rt > (tempmean-3*tempsd))), ]
    pipelines_data[["AAR3E"]] <- pipelines_data[["AAR3N"]][-c(which(pipelines_data[["AAR3N"]]$correct == 0) + 1), ]
  
    # log transformed versions
    
    pipelines_data[["AALNN"]] <- pipelines_data[["RNN"]]
    pipelines_data[["AALNN"]]$rt[pipelines_data[["AALNN"]]$correct == 1] <- log10(pipelines_data[["AALNN"]]$rt[pipelines_data[["AALNN"]]$correct == 1])
    
    tempmean <- mean(pipelines_data[["AALNN"]]$rt[pipelines_data[["AALNN"]]$correct == 1])
    tempsd <- sd(pipelines_data[["AALNN"]]$rt[pipelines_data[["AALNN"]]$correct == 1])
    
    pipelines_data[["AALNE"]] <- pipelines_data[["AALNN"]][-c(which(pipelines_data[["AALNN"]]$correct == 0) + 1), ]
    pipelines_data[["AAL2N"]] <- pipelines_data[["AALNN"]][c(which(pipelines_data[["AALNN"]]$rt < (tempmean+2*tempsd) & pipelines_data[["AALNN"]]$rt > (tempmean-2*tempsd))), ]
    pipelines_data[["AAL2E"]] <- pipelines_data[["AAL2N"]][-c(which(pipelines_data[["AAL2N"]]$correct == 0) + 1), ]
    pipelines_data[["AAL3N"]] <- pipelines_data[["AALNN"]][c(which(pipelines_data[["AALNN"]]$rt < (tempmean+3*tempsd) & pipelines_data[["AALNN"]]$rt > (tempmean-3*tempsd))), ]
    pipelines_data[["AAL3E"]] <- pipelines_data[["AAL3N"]][-c(which(pipelines_data[["AAL3N"]]$correct == 0) + 1), ]
  
    # inverse transformed versions
    
    pipelines_data[["AAINN"]] <- pipelines_data[["RNN"]]
    pipelines_data[["AAINN"]]$rt[pipelines_data[["AAINN"]]$correct == 1] <- 1/(pipelines_data[["AAINN"]]$rt[pipelines_data[["AAINN"]]$correct == 1])
    
    tempmean <- mean(pipelines_data[["AAINN"]]$rt[pipelines_data[["AAINN"]]$correct == 1])
    tempsd <- sd(pipelines_data[["AAINN"]]$rt[pipelines_data[["AAINN"]]$correct == 1])
    
    pipelines_data[["AAINE"]] <- pipelines_data[["AAINN"]][-c(which(pipelines_data[["AAINN"]]$correct == 0) + 1), ]
    pipelines_data[["AAI2N"]] <- pipelines_data[["AAINN"]][c(which(pipelines_data[["AAINN"]]$rt < (tempmean+2*tempsd) & pipelines_data[["AAINN"]]$rt > (tempmean-2*tempsd))), ]
    pipelines_data[["AAI2E"]] <- pipelines_data[["AAI2N"]][-c(which(pipelines_data[["AAI2N"]]$correct == 0) + 1), ]
    pipelines_data[["AAI3N"]] <- pipelines_data[["AAINN"]][c(which(pipelines_data[["AAINN"]]$rt < (tempmean+3*tempsd) & pipelines_data[["AAINN"]]$rt > (tempmean-3*tempsd))), ]
    pipelines_data[["AAI3E"]] <- pipelines_data[["AAI3N"]][-c(which(pipelines_data[["AAI3N"]]$correct == 0) + 1), ]
  
    # square root transformed versions
    
    pipelines_data[["AASNN"]] <- pipelines_data[["RNN"]]
    pipelines_data[["AASNN"]]$rt[pipelines_data[["AASNN"]]$correct == 1] <- sqrt(pipelines_data[["AASNN"]]$rt[pipelines_data[["AASNN"]]$correct == 1])
    
    tempmean <- mean(pipelines_data[["AASNN"]]$rt[pipelines_data[["AASNN"]]$correct == 1])
    tempsd <- sd(pipelines_data[["AASNN"]]$rt[pipelines_data[["AASNN"]]$correct == 1])
    
    pipelines_data[["AASNE"]] <- pipelines_data[["AASNN"]][-c(which(pipelines_data[["AASNN"]]$correct == 0) + 1), ]
    pipelines_data[["AAS2N"]] <- pipelines_data[["AASNN"]][c(which(pipelines_data[["AASNN"]]$rt < (tempmean+2*tempsd) & pipelines_data[["AASNN"]]$rt > (tempmean-2*tempsd))), ]
    pipelines_data[["AAS2E"]] <- pipelines_data[["AAS2N"]][-c(which(pipelines_data[["AAS2N"]]$correct == 0) + 1), ]
    pipelines_data[["AAS3N"]] <- pipelines_data[["AASNN"]][c(which(pipelines_data[["AASNN"]]$rt < (tempmean+3*tempsd) & pipelines_data[["AASNN"]]$rt > (tempmean-3*tempsd))), ]
    pipelines_data[["AAS3E"]] <- pipelines_data[["AAS3N"]][-c(which(pipelines_data[["AAS3N"]]$correct == 0) + 1), ]
  
  # group 2: across subjects, within conditions
  
  # group 3: within subjects, within conditions
  
  # group 4: within subjects, across conditions
  
  
      # create index of rows in which the level changes
      #levelindex <- c(1,which(data_nback$level != dplyr::lag(data_nback$level)),nrow(data_nback))
      
      #for (i in 1:(length(levelindex)-1)) {
        
        #RT_L[levelindex[i]:(levelindex[i+1]-1),"rt"] <- log10(RT_L[levelindex[i]:(levelindex[i+1]-1),"rt"])
        #RT_L[levelindex[i]:(levelindex[i+1]-1),"rt"] <- 1/(RT_L[levelindex[i]:(levelindex[i+1]-1),"rt"])
        #RT_L[levelindex[i]:(levelindex[i+1]-1),"rt"] <- sqrt(RT_L[levelindex[i]:(levelindex[i+1]-1),"rt"])
      #}
...
The R Markdown file used to analyze the data and write this document is freely available at github.com/ChScheffel/CERED.
The data can be downloaded from osf.io/vnj8x/.

## Questionnaires
To assess perceived task load, we will use the 6-item NASA Task Load Index (NASA-TLX) [@Hart1988].
In the NASA-TLX, participants evaluate their subjective perception of mental load, physical load, effort, frustration, performance, and time pressure during the task on a 20-point scale.
Need for Cognition will be assessed using the 16-item short form of the Need for Cognition Scale [@Bless1994; @Cacioppo1984].
Responses to each item (e.g., "Thinking is not my idea of fun", recoded) will be recorded on a 7-point Likert scale ranging from -3 (completely disagree) to +3 (completely agree).
The scale shows comparably high internal consistency [Cronbach’s $\alpha$ > .80; @Bless1994; @Fleischhauer2010].
Several other personality questionnaires will be used in this study but are the topic of the registered report for the second lab session regarding ED in emotion regulation tasks and will not be analyzed here.
...
Each participant completed two sessions in the lab one week apart, and a battery of personality questionnaires two days after the second session.
...
The levels were referred to by color (1-back black, 2-back red, 3-back blue, 4-back green) to avoid anchor effects in the ED procedure.
After each level, participants filled out the NASA-TLX on a tablet to provide a state measure of cognitive effort.
...
The level and its monetary value were presented on screen and participants completed one run, but this was only to incentivise truthful behavior in the ED procedure, so the n-back data of this part was not analyzed.
...
The paradigm followed the same structure of task and ED procedure, but the emotion regulation strategy at the end was not randomly chosen, instead participants could decide which strategy they wanted to use again.
...
Participants received 30€ in total or course credit for participation.
...
## Analysis
We aimed to conduct all analysis as described in Westbrook et al. [-@Westbrook2013], but the level of detail was not always sufficient, so there might be deviations in terms of data cleaning and degrees of freedom in the statistical procedures.
...
Reaction time (RT) data was trimmed by excluding all trials with responses faster than 200 ms, as the relevant cognitive processes could not have been completed before [@Ashby1980; @Whelan2008; @Berger2021].
Error- and post-error trials were excluded, because RT on the latter trials is longer due to more cautious behavior [@Dutilh2012; @Houtman2012].
#TODO: Tolle Artikel, die schaue ich mir auch noch mal genauer an! Hast du die beiden exclusions schon im script umgesetzt? Falls ja, in welcher Zeile, ich kann es nicht finden...
Declining performance was investigated by calculating a repeated measures analysis of variance (rmANOVA) with three paired contrasts comparing *d'* between two levels of 2-, 3-, and 4-back at a time.
Another rmANOVA with three paired contrasts was computed to compare the mean RT between two levels of 2-, 3-, and 4-back at a time.
To investigate changes in NASA-TLX ratings, six rmANOVAs were computed, one for each NASA-TLX subscale, and each with six paired contrasts comparing the ratings between two levels of 1-, 2-, 3-, and 4-back at a time.
...
A rmANOVA with six paired contrasts was computed, comparing the SVs between two levels of 1-, 2-, 3-, and 4-back at a time.
Tukey method was used for the paired contrasts of each rmANOVA, including *p*-value adjustment.
...
The association of ED and NFC was examined using three separate regressions.
One regression used the AUC of each participant's SVs to predict their NFC score.
The second regression additionally included the mean of the NASA-TLX subscales' AUCs of each participant as a predictor.
And the last regression **insert method**.
...
```{r Hypothesis_1a, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  levelindex <- c(1,which(data_RT$level != dplyr::lag(data_RT$level)),nrow(data_RT))
  ...
  for (i in 1:(length(levelindex)-1)) {
    ...
    num_targets <- length(which(data_RT$target[c(levelindex[i]:(levelindex[i+1])-1)] == 1))
    num_nontargets <- length(which(data_RT$target[c(levelindex[i]:(levelindex[i+1])-1)] == 0))
    
    hits <- length(which(data_RT$target[c(levelindex[i]:(levelindex[i+1])-1)] == 1 &
                           data_RT$correct[c(levelindex[i]:(levelindex[i+1])-1)] == 1) + (levelindex[i]-1))
    falsealarms <- length(which(data_RT$target[c(levelindex[i]:(levelindex[i+1])-1)] == 0 &
                           data_RT$correct[c(levelindex[i]:(levelindex[i+1])-1)] == 0) + (levelindex[i]-1))
    newdata <- data.frame(subject = data_RT$subject[levelindex[i]],
                          level = data_RT$level[levelindex[i]],
  ...
  # repeated measures ANOVA with three linear contrasts, contrasting the d’ of two n-back levels (2,3,4) at a time
      ...
      hypothesis1a_contrasts <- pairs(hypothesis1a_emm)
...
### Associations of subjective and objective task load measures
We first looked at how performance measures and NASA-TLX ratings change as n-back levels increase.
The rmANOVA revealed decreasing performance *d'* with increasing n-back level (`r apa_print(hypothesis1a_rmanova)$full_result`).
Paired contrasts showed that *d'* was smaller for *...(and put in element 1/2/3 ->)* (`r apa_print(hypothesis1a_contrasts)$statistic[1]`).
...
```{r Hypothesis_1b, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  for (i in 1:(length(levelindex)-1)) {
    ...
    meanrt <- mean(data_RT$rt[which(data_RT$correct[c(levelindex[i]:(levelindex[i+1])-1)] == 1)
  ...
  # ANOVA with three linear contrasts, contrasting the mean rt of two n-back levels (2,3,4) at a time
      ...
      hypothesis1b_emm <- emmeans(object = hypothesis1b_rmanova$aov,
                                  specs = "level")
      # calculate pairwise comparisons on estimated marginal means
      hypothesis1b_contrasts <- pairs(hypothesis1b_emm)
...
Likewise, RT increased with increasing n-back level (`r apa_print(hypothesis1b_rmanova)$full_result`).
Here, RTs were longer for *...(and put in element 1/2/3 ->)* (`r apa_print(hypothesis1b_contrasts)$statistic[1]`).
Overall, the decreases in *d'* and increases in RT indicate the increase in objective task load with higher n-back levels.
...
All subscales showed increases from 1-back to 4-back (mental: `r apa_print(hypothesis1c_mental_contrasts)$statistic$X1_X4`, physical: `r apa_print(hypothesis1c_pysical_contrasts)$statistic$X1_X4`, time: `r apa_print(hypothesis1c_time_contrasts)$statistic$X1_X4`, performance: `r apa_print(hypothesis1c_performance_contrasts)$statistic$X1_X4`, effort: `r apa_print(hypothesis1c_effort_contrasts)$statistic$X1_X4`, frustration: `r apa_print(hypothesis1c_frustration_contrasts)$statistic$X1_X4`).
...
```{r Hypothesis_2a, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  hypothesis2a_emm <- emmeans(object = hypothesis2a_rmanova$aov,
                              specs = "level")
  # calculate pairwise comparisons on estimated marginal means
  hypothesis2a_contrasts <- pairs(hypothesis2a_emm)
...
### Changes in subjective values depending on n-back level
While the rmANOVA indicated significant differences between SVs depending on n-back level (`r apa_print(hypothesis2a_rmanova)$full_result`), paired contrasts indicated that this was not the case for every pair of n-back levels.
SVs were higher for *n*-back than for *n*-back *insert row!!* (`r apa_print(hypothesis2a_contrasts)$statistic$X1_X4`) and for *n*-back (`r apa_print(hypothesis2a_contrasts)$statistic$X1_X4`).
The SVs of all other n-back level pairs were not significantly different.
...
```{r Hypothesis_2b, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  # detach afex in order to user lmerTest
  detach("package:afex", unload = TRUE)
  
  # get all relevant variables -------------------------------------------------
    
  h2b_data <- cbind(data_SV, dprime["d"], rt["meanrt"])
  ...
  # d'
  h2b_data$d.mwc <- ave(h2b_data$d, h2b_data$subject, FUN = function(x) mean(x, na.rm = T))
  h2b_data$d.cwc <- h2b_data$d - h2b_data$d.mwc
  
  # mRT
  h2b_data$meanrt.mwc <- ave(h2b_data$meanrt, h2b_data$subject, FUN = function(x) mean(x, na.rm = T))
  h2b_data$meanrt.cwc <- h2b_data$meanrt - h2b_data$meanrt.mwc
  
  # level
  h2b_data$level.mwc <- ave(h2b_data$level, h2b_data$subject, FUN = function(x) mean(x, na.rm = T))
  h2b_data$level.cwc <- h2b_data$level - h2b_data$level.mwc
  ...
  m1_h2b <- lmer(sv ~ level.cwc + d.cwc + meanrt.cwc + (level.cwc|subject), 
                 data = h2b_data,
                 REML = T) # parameters estimated by maximizing restricted log-likelihood
  
  # (1) if a warning message is produced, check random effects
    summary(m1_h2b)$varcor
    # (11) if all SDs are different from 0 and correlations look good (unequal to zero)
      # this message should be a false positive and this model can be reported
    # (12) if (11) is not true, check all available optimizers
      n_cores <- parallel::detectCores()
      opt_cl <- makeCluster(n_cores, type = "SOCK")
      clusterEvalQ(opt_cl, library("lme4"))
      clusterExport(opt_cl, "h2b_data")
      m1_h2b_all <- allFit(m1_h2b, parallel = "snow", ncpus = n_cores, cl = opt_cl) 
      stopCluster(opt_cl)
      summary(m1_h2b_all)$msgs
      # (121) if an optimizer returns NULL, use this optimizer and recalculate model to get the final model
  
  # (2) if (121) is not true OR a singular fit message occurs, check for potential outliers and recalculate
    potential_outliers_m1_h2b <- subset(h2b_data, abs(scale(resid(m1_h2b))) > 3)    
    percentage_outliers <- 100/nrow(h2b_data) * nrow(potential_outliers_m1_h2b)
    # get new subset excluding outliers
    h2b_data_norm <- droplevels(subset(h2b_data, abs(scale(resid(m1_h2b))) < 3))
    # start with (1) again
  
  # (3) if (1) and (2) do not help, start reducing the model stepwise
  
  ## plot model fit of the final model ----
  plot_model(mfinal_h2b, type = "diag")
  
  # FIXME: do we want to include  a table like Table 3. in Westbrook et al.?
    # if we decide to include a table of mlm results, I propose to include a table with
    # term/parameter - Beta - SE - p-value - random effects (SD)
  
  # TODO: Corinna go on here :) 
    # -> get results for reporting
    # -> get means for plotting, plot results (if requested)
    # -> write methods
    # -> write results
    
  # TODO: it would be easy to integrate NFC as level 2 predictor into the analysis
    # to identify the personality influence on sv
    # it goes in the same direction as hypothesis 3a, but would hold some more information
    # a corresponding hypothesis could be
    # "SVs decline stronger with increasing load level for individuals with low compared to high NFC scores" 
    # What do you think?
  

```

### Changes in subjective values depend reliably on n-back level

```{r AUC_demo, echo=FALSE, message=FALSE, warning=FALSE}

require(gtools)
demo_data = data.frame(level = rep(1:4,24))
sv = permutations(n = 4, r = 4, v = c(0.1,0.4,0.85,1))
m = 1
for (n in 1:(nrow(demo_data)/4)) {
  demo_data$subject[m:(m+3)] = rep(n,4)
  #demo_data$sv[m:(m+3)] = sv[n,]
  demo_data$sv[m:(m+3)] = sample(x = c(0.11,0.38,0.45,0.53,0.70,0.85,0.90,1.00), size = 4, replace = FALSE, prob = c(0.025,0.025,0.05,0.05,0.05,0.05,0.05,0.7))
  m = m + 4
}
subjectindex <- c(1,which(demo_data$subject != dplyr::lag(demo_data$subject)),nrow(demo_data))
demo_sv <- data.frame(subject = character(), auc_sv = double(),
                      ascent_mean = double(), ascent_sum = double(), auc_x_ascentmean = double(), auc_x_ascentsum = double(),
                      auc_westbrook = double())
for (i in 1:(length(subjectindex)-1)) {
  sv_sorted <- sort(demo_data$sv[subjectindex[i]:(subjectindex[i]+3)])
  newdata <- data.frame(subject = demo_data$subject[subjectindex[i]],
                        auc_sv = auc(demo_data$level[subjectindex[i]:(subjectindex[i]+3)], demo_data$sv[subjectindex[i]:(subjectindex[i]+3)],
                                   method = "trapezoid"),
                        auc_sv_sorted = (0.5 * (sv_sorted[1] + sv_sorted[2]) +
                                           0.5 * (sv_sorted[2] + sv_sorted[3]) +
                                           0.5 * (sv_sorted[3] + sv_sorted[4])),
                        ascent_mean = ((demo_data$sv[subjectindex[i]+3] - demo_data$sv[subjectindex[i]+2]) +
                                        (demo_data$sv[subjectindex[i]+2] - demo_data$sv[subjectindex[i]+1]) +
                                        (demo_data$sv[subjectindex[i]+1] - demo_data$sv[subjectindex[i]]))/3,
                        ascent_sum = (demo_data$sv[subjectindex[i]+3] - demo_data$sv[subjectindex[i]+2]) +
                                        (demo_data$sv[subjectindex[i]+2] - demo_data$sv[subjectindex[i]+1]) +
                                        (demo_data$sv[subjectindex[i]+1] - demo_data$sv[subjectindex[i]]),
                        auc_x_ascentmean = auc(demo_data$level[subjectindex[i]:(subjectindex[i]+3)], demo_data$sv[subjectindex[i]:(subjectindex[i]+3)],
                                               method = "trapezoid") * (((demo_data$sv[subjectindex[i]+3] - demo_data$sv[subjectindex[i]+2]) +
                                        (demo_data$sv[subjectindex[i]+2] - demo_data$sv[subjectindex[i]+1]) +
                                        (demo_data$sv[subjectindex[i]+1] - demo_data$sv[subjectindex[i]]))/3),
                        auc_x_ascentsum = auc(demo_data$level[subjectindex[i]:(subjectindex[i]+3)], demo_data$sv[subjectindex[i]:(subjectindex[i]+3)],
                                               method = "trapezoid") * ((demo_data$sv[subjectindex[i]+3] - demo_data$sv[subjectindex[i]+2]) +
                                        (demo_data$sv[subjectindex[i]+2] - demo_data$sv[subjectindex[i]+1]) +
                                        (demo_data$sv[subjectindex[i]+1] - demo_data$sv[subjectindex[i]])),
                        auc_westbrook = (0.5 * (sv_sorted[1] + sv_sorted[2]) +
                                           0.5 * (sv_sorted[1] + sv_sorted[3]) +
                                           0.5 * (sv_sorted[1] + sv_sorted[4]))/3)
  demo_sv <- rbind(demo_sv, newdata)
}
demo_cor <- cor(demo_sv$auc_sv, demo_sv$auc_westbrook)

ggplot(demo_data, aes(x=level, y=sv, group=subject)) +
  geom_line(aes(color=subject), size = 1)

#library(data.table) for the melt function
ggplot(melt(setDT(demo_sv), id.vars = "subject", variable.name = "measure"), aes(fill=measure,y=value,x=subject)) + geom_bar(position = "dodge", stat = "identity")
```
...
```{r Hypothesis_3a, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  h3_data <- data.frame(subject = character(), auc_sv = double(), auc_sv_unsorted = double())
  ...
  for (i in 1:(length(subjectindex)-1)) {
    ...
    newdata <- data.frame(subject = data_SV$subject[subjectindex[i]],
                          ...
                          auc_sv_unsorted = (0.5 * (data_SV$sv[subjectindex[i]] + data_SV$sv[subjectindex[i]+1]) +
                                               0.5 * (data_SV$sv[subjectindex[i]+1] + data_SV$sv[subjectindex[i]+2]) +
                                               0.5 * (data_SV$sv[subjectindex[i]+2] + data_SV$sv[subjectindex[i]+3])))
  ...
  h3_data <- merge(h3_data, data_nfc)
  ...
  hypothesis3a <- summary(lm(h3_data$nfc ~ h3_data$auc_sv))
  
  

```

The AUC of SVs predicted a significant amount of variance in individual NFC scores (`r apa_print(hypothesis3a)$statistic$modelfit$r2`, `r apa_print(hypothesis3a)$estimate$modelfit$r2`).
An increase in one AUC value corresponded, on average, to an increase of `r round(hypothesis3a$coefficients[2], digits = 1)` on the NFC scale (`r apa_print(hypothesis3a)$estimate$h3_data_auc_sv`).

```{r Hypothesis_3b, echo=FALSE, message=FALSE, warning=FALSE}

  # H3b: NASA-TLX scores do not predict individual NCS scores.
  ...
  hypothesis3b <- summary(lm(h3_data$nfc ~ h3_data$auc_sv + h3_data$auc_ntlx))

```

When including the AUC of all NASA-TLX subscales as a predictor, it could not explain any variance in NFC beyond the one explained by the SVs (`r apa_print(hypothesis3b)$statistic$modelfit$r2`, `r apa_print(hypothesis3b)$estimate$modelfit$r2`).

```{r Hypothesis_3c, echo=FALSE, message=FALSE, warning=FALSE}

  # H3c: When putting the SVs in descending order before computing the AUC, the prediction of NFC scores by SVs can be improved significantly.

  # prepare empty data frame for the loop to feed into

  h3c_data <- data.frame(subject = character(), auc_sv_sorted = double())
  ...
  for (i in 1:(length(subjectindex)-1)) {
    ...
    newdata <- data.frame(subject = data_SV$subject[subjectindex[i]],
                          auc_sv_sorted = auc(data_SV$level[subjectindex[i]:(subjectindex[i+1]-1)], data_SV$sv[subjectindex[i]:(subjectindex[i+1]-1)],
                                   method = "trapezoid", sort = TRUE))
    h3c_data <- rbind(h3c_data, newdata)
    
  }

  # merge the NFC data frame into the AUC data frame by row
  
  h3_data <- merge(h3_data, h3c_data)
  
  # remove temporary variable
  
  remove(h3c_data)

  # compute linear regression to predict NCS based on SV AUC
  
  hypothesis3c <- summary(lm(h3_data$nfc ~ h3_data$auc_sv_sorted))
