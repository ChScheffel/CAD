...
```{r import, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  # remove trials with RT < 200 ms from the n-back data and exclude trials after incorrect trials
  data_RT <- data_nback[-c(which(data_nback$rt < 0.2)), ]
  data_RT <- data_RT[-c(which(data_RT$correct == 0) + 1), ]
...
```{r preprocessing, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  for (i in 1:length(pipelines_data)) {
    ...
    if (pipeline_name[[1]][1] == "A" & pipeline_name[[1]][2] == "A") {
        ...
        for (x in 1:4) {
            ...
            if (pipeline_name[[1]][3] == "L") {
              ...
              pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x] <- log10(pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x])
            
            } else if (pipeline_name[[1]][3] == "I") {
            
              pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x] <- 1/(pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x])
            
            } else if (pipeline_name[[1]][3] == "S") {
            
              pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x] <- sqrt(pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x])

            }
            
            # compute mean and standard deviation
          
            pipemean <- mean(pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x], na.rm = TRUE)
            pipesd <- sd(pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x], na.rm = TRUE)
            
            # the third decision is the outlier exclusion
          
            if (pipeline_name[[1]][4] == "2") {
              
              ifelse(length(which(pipelines_data[[i]]$level == x &
                                    (pipelines_data[[i]]$rt > (pipemean + 2 * pipesd) | pipelines_data[[i]]$rt < (pipemean - 2 * pipesd)))) > 0,
                     pipelines_data[[i]] <-
                       pipelines_data[[i]][-c(which(pipelines_data[[i]]$level == x &
                                                      (pipelines_data[[i]]$rt > (pipemean + 2 * pipesd) | pipelines_data[[i]]$rt < (pipemean - 2 * pipesd)))), ],
                     pipelines_data[[i]] <- pipelines_data[[i]])
                  
            } else if (pipeline_name[[1]][4] == "3") {
              
              ifelse(length(which(pipelines_data[[i]]$level == x &
                                    (pipelines_data[[i]]$rt > (pipemean + 3 * pipesd) | pipelines_data[[i]]$rt < (pipemean - 3 * pipesd)))) > 0,
                     pipelines_data[[i]] <-
                       pipelines_data[[i]][-c(which(pipelines_data[[i]]$level == x &
                                                    (pipelines_data[[i]]$rt > (pipemean + 3 * pipesd) | pipelines_data[[i]]$rt < (pipemean - 3 * pipesd)))), ],
                     pipelines_data[[i]] <- pipelines_data[[i]])
            }
        }
        ...
        for (x in 1:4) {
          ...
          for (y in 1:length(table(pipelines_data[[i]]$subject))) {
            ...
            if (pipeline_name[[1]][3] == "L") {
              ...
              pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x
                                     & pipelines_data[[i]]$subject == pipesubject] <- log10(pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x
                                                                                                                   & pipelines_data[[i]]$subject == pipesubject])
            
            } else if (pipeline_name[[1]][3] == "I") {
            
              pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x
                                     & pipelines_data[[i]]$subject == pipesubject] <- 1/(pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x
                                                                                                                & pipelines_data[[i]]$subject == pipesubject])
            
            } else if (pipeline_name[[1]][3] == "S") {
            
              pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x
                                     & pipelines_data[[i]]$subject == pipesubject] <- sqrt(pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x
                                                                                                                  & pipelines_data[[i]]$subject == pipesubject])
            ...
            pipemean <- mean(pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x & pipelines_data[[i]]$subject == pipesubject], na.rm = TRUE)
            pipesd <- sd(pipelines_data[[i]]$rt[pipelines_data[[i]]$level == x & pipelines_data[[i]]$subject == pipesubject], na.rm = TRUE)
            
            # the third decision is the outlier exclusion
          
            if (pipeline_name[[1]][4] == "2") {
              
              ifelse(length(which(pipelines_data[[i]]$level == x & pipelines_data[[i]]$subject == pipesubject &
                                    (pipelines_data[[i]]$rt > (pipemean + 2 * pipesd) | pipelines_data[[i]]$rt < (pipemean - 2 * pipesd)))) > 0,
                     pipelines_data[[i]] <-
                       pipelines_data[[i]][-c(which(pipelines_data[[i]]$level == x & pipelines_data[[i]]$subject == pipesubject &
                                                      (pipelines_data[[i]]$rt > (pipemean + 2 * pipesd) | pipelines_data[[i]]$rt < (pipemean - 2 * pipesd)))), ],
                     pipelines_data[[i]] <- pipelines_data[[i]])
      ...
      # apply the transformations and exclusions within subjects but across levels
        ...
        for (y in 1:length(table(pipelines_data[[i]]$subject))) {
            ...
            if (pipeline_name[[1]][3] == "L") {
              ...
              pipelines_data[[i]]$rt[pipelines_data[[i]]$subject == pipesubject] <- log10(pipelines_data[[i]]$rt[pipelines_data[[i]]$subject == pipesubject])
            
            } else if (pipeline_name[[1]][3] == "I") {
            
              pipelines_data[[i]]$rt[pipelines_data[[i]]$subject == pipesubject] <- 1/(pipelines_data[[i]]$rt[pipelines_data[[i]]$subject == pipesubject])
            
            } else if (pipeline_name[[1]][3] == "S") {
            
              pipelines_data[[i]]$rt[pipelines_data[[i]]$subject == pipesubject] <- sqrt(pipelines_data[[i]]$rt[pipelines_data[[i]]$subject == pipesubject])

            }
            
            # compute mean and standard deviation
          
            pipemean <- mean(pipelines_data[[i]]$rt[pipelines_data[[i]]$subject == pipesubject], na.rm = TRUE)
            pipesd <- sd(pipelines_data[[i]]$rt[pipelines_data[[i]]$subject == pipesubject], na.rm = TRUE)
            
            # the third decision is the outlier exclusion
          
            if (pipeline_name[[1]][4] == "2") {
              
              ifelse(length(which(pipelines_data[[i]]$subject == pipesubject &
                                    (pipelines_data[[i]]$rt > (pipemean + 2 * pipesd) | pipelines_data[[i]]$rt < (pipemean - 2 * pipesd)))) > 0,
                     pipelines_data[[i]] <-
                       pipelines_data[[i]][-c(which(pipelines_data[[i]]$subject == pipesubject &
                                                      (pipelines_data[[i]]$rt > (pipemean + 2 * pipesd) | pipelines_data[[i]]$rt < (pipemean - 2 * pipesd)))), ],
                     pipelines_data[[i]] <- pipelines_data[[i]])
    ...
    # the 200-ms-exclusion and the post-error-exclusion are done outside of the A/A-A/W-W/W-W/A-conditions because they are independent of dimension:
    
    # exclude RTs fast than 200 ms after stimulus onset if the fourth letter in the data frame name is a "T"
    
    if (pipeline_name[[1]][4] == "T") {
      
      pipelines_data[[i]] <- pipelines_data[[i]][-c(which(pipelines_data[[i]]$rt < 0.2)), ]
      
    }
    
    # exclude post-error trials if the fifth letter in the data frame name is an "E"
    # also making sure that there are even any error-/post-error-rows left to perform the exclusion, otherwise you get an error message
    
    if (pipeline_name[[1]][5] == "E") {
      
      ifelse(nrow(pipelines_data[[i]][-c(which(pipelines_data[[i]]$correct == 0) + 1), ]) > 0,
             pipelines_data[[i]] <- pipelines_data[[i]][-c(which(pipelines_data[[i]]$correct == 0) + 1), ],
             pipelines_data[[i]] <- pipelines_data[[i]])
...
Reaction time (RT) data was trimmed by excluding all trials with responses faster than 200 ms, as the relevant cognitive processes could not have been completed before [@Whelan2008; @Berger2021].
...
To ensure the validity of the multi-level model, we conducted a specification curve analysis [@Simonsohn2020], which included 92 possible preprocessing pipelines of the RT data.
These pipelines specified which transformation has been applied (none, log, inverse, or square-root), which outliers have been excluded (none, $2 SD$ from the mean, $3 SD$ from the mean, RTs below 200 ms), whether post-error trials have been excluded or not, and across which dimensions the transformations and exclusions have been applied (across/within subjects and across/within n-back levels).
The multi-level model was run with each of the 92 pipelines, which also included our main pipeline (untransformed data, exclusion of RTs below 200 ms, removal of post-error trials, no decision regarding dimension necessary).
The ratio of pipelines that lead to significant versus non-significant effects provides and indication of how robust the effect actually is.

The association of ED and NFC was examined with a regression using the AUC of each participant's SVs to predict their NFC score.
A second regression additionally included the mean of the NASA-TLX subscales' AUCs of each participant as a predictor.
Since we did not have a fixed SV of 1 for 1-back, we could not apply the 'AUC' computation of Westbrook et al. [-@Westbrook2013], which was the mean of the AUCs of the SVs of each higher n-back level and 1-back, yielding values between 0 and 1.
Consequently, we decided on a different way of quantifying the individual degree of ED.
A classic AUC cannot differentiate between a subject who prefers 1-back and a subject who prefers 4-back, but can reflect the overall willingness to exert effort.
This is the opposite for the sum of the ascent between SVs.
Therefore, we multiplied both indicators to arrive at a value reflecting both degree and direction of preference, called $AxAUC$.
