...
```{r SpecificationCurveAnalysis, echo=FALSE, message=FALSE, warning=FALSE, fig.show='hide'}
  ...
  sca_results <- data.frame(pipeline = character(), beta_level = double(), p_level = double(),
                            beta_nfc = double(), p_nfc = double())
  ...
  for (i in 1:length(pipelines_data)) {
    ...
    newdata <- data.frame(pipeline = names(pipelines_data[i]),
                          beta_level = summary(m1_sca)$coefficients[2,1],
                          p_level = summary(m1_sca)$coefficients[2,5],
                          beta_nfc = summary(m1_sca)$coefficients[3,1],
                          p_nfc = summary(m1_sca)$coefficients[3,5])
...
```{r SCA_plot, echo=FALSE, message=FALSE, warning=FALSE, fig.show='hide'}
  ...
  # sort the data frame by the fixed effects estimate of the predictor 'level'

  sca_results <- sca_results[order(beta_level), ] 
