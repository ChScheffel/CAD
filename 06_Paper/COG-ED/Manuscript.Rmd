...
```{r Hypothesis_3a, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  h3_data <- data.frame(subject = character(), auc_sv = double())
...
```{r Hypothesis_3c, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  h3c_data <- data.frame(subject = character(), auc_sv = double())
  ...
  hypothesis3c <- summary(lm(h3_data$nfc ~ h3_data$auc_sv))

```

However, when only taking the AUC of the mental and the AUC of the effort subscale of the NASA-TLX into account, it explained variance in NFC scores beyond SVs (`r apa_print(hypothesis3c)$statistic$modelfit$r2`, `r apa_print(hypothesis3c)$estimate$modelfit$r2`).
