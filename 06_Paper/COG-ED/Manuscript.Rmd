...
```{r Hypothesis_2b_2c, echo=FALSE, message=FALSE, warning=FALSE, fig.show='hide'}
  ...
  #create table
  h2b_ss.table <- as_huxtable(m1_h2b.ss)
  h2b_ss.table[1,] <- c("Value of NFC", "Slope of N-Back Level", "Conditional Intercept")
  h2b_ss.table <- h2b_ss.table[-2,]
  h2b_ss.table[2:4,1] <- c("- 1 SD", "Mean", "+ 1 SD")  
  
  # calculate Johnson-Neyman intervals
  h2b_jn <- johnson_neyman(model = m1_h2b, pred = level.cwc, modx = nfc.cgm, control.fdr = T)
  h2b_jn.int <- h2b_jn$bounds
  
  # save plot
  h2b_jn.plot <- h2b_jn$plot
  
  # create interaction plot
  h2b_ss.plot <- interact_plot(m1_h2b, pred = level.cwc, modx = nfc.cgm, centered = "none",
...
N-back level is a reliable predictor of subjective value (`r apa_print(m1_h2b)$full_result$level_cwc`), even after controlling for declining task performance (signal detection d’ and reaction time) as well as correct and post-correct answers. 
The results of the multilevel model are detailed in Table X. 
`r replication_table`
...
However, NFC has no statistically significant effect on subjective value (`r apa_print(m1_h2b)$full_result$nfc_cgm`).
The simple slopes analysis revealed a significant slope of n-back level for three different expressions of NFC (Table X, Figure X).
`r h2b_ss.table`
`r h2b_ss.plot`
The Johnson-Neyman plot (Figure x) visualizes the significance of the slope of n-back level at different values of NFC. The Johnson-Neyman interval is [`r h2b_jn.int`]. Consequently, the slope of n-back level is not significant for individuals with very low NFC values.    
`r h2b_jn.plot`
